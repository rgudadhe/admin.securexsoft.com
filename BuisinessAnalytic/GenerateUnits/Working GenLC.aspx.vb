...
Partial Class GenLC_New
                        ...
                        Else
                            ...
                            If Trim(DRRec1("Mode").ToString) = "S" Or Trim(DRRec1("Mode").ToString) = "" Then
                                ...
                                Try
                                    ...
                                    If DRRec3.HasRows = True Then
                                        UDFound = True
                                        If DRRec3.Read Then
                                            strQuery = "DELETE FROM ETS.dbo.tblbillinglines WHERE BillAccID ='11111111-1111-1111-1111-111111111111' AND Transcriptionid in (Select TranscriptionID FROM SecureWeb.dbo.tblTranscriptionClientMain where AccountID = '" & DRRec1("AccountID").ToString & "'  and datediff(day,datemodified, '" & edate & "') between 0 and datediff(day,'" & sdate & "','" & edate & "')  ) "
                            ...
                            ElseIf Trim(DRRec1("Mode").ToString) = "LC" Then
                                ...
                                If ErrFound = False Then
                                    strQuery = "Select * from ETS.dbo.tblAccountsLocations A LEFT OUTER JOIN Secureweb.dbo.BillDetails B ON A.AccountID= B.AccountID and A.TrackID = B.SubActID where A.AccountID = '" & DRRec1("AccountID").ToString & "' and B.SubActID IS NULL  "
                                    'Response.Write(strQuery)
                                                            ...
                                                            Try
                                                                ...
                                                                If DRRec4.HasRows = True Then
                                                                    ...
                                                                    If DRRec4.Read Then
                                                                        ...
                                                                        strQuery = "Select distinct T.*, M.Type,M.TAT,CASE WHEN M.Priority = 1 AND (DATEDIFF(MINUTE, M.DateCreated, ISNULL(M.DateModified, GETDATE()))/60) < M.TAT THEN 'True' ELSE 'False' END as Priority,M.DictatorID,M.Jobnumber from SecureWeb.dbo.tblTranscriptionClientMain M Left Outer Join tblTransLC T ON T.TranscriptionID = M.TranscriptionID where T.TranscriptionID is not NULL and M.AccountID = '" & DRRec1("AccountID").ToString & "'   and datediff(day,M.datemodified, '" & edate & "') between 0 and datediff(day,'" & sdate & "','" & edate & "')   and M.TAT ='" & TAT & "' order by JobNumber"
                                                                        'Response.Write(strQuery)
                                                                        Dim i As Integer
                                                                        i = 0
                                                                        Dim SQLCmd5 As New SqlCommand(strQuery, New SqlConnection(strConn))
                                ...
                                strQuery = "Select distinct T.*, M.Type,M.Location,CASE WHEN M.Priority = 1 AND (DATEDIFF(MINUTE, M.DateCreated, ISNULL(M.DateModified, GETDATE()))/60) < M.TAT THEN 'True' ELSE 'False' END as Priority,M.DictatorID,M.Jobnumber from SecureWeb.dbo.tblTranscriptionClientMain M  Left Outer Join tblTransLC T ON T.TranscriptionID = M.TranscriptionID where T.TranscriptionID is not NULL and M.AccountID = '" & DRRec1("AccountID").ToString & "'   and datediff(day,M.datemodified, '" & edate & "') between 0 and datediff(day,'" & sdate & "','" & edate & "')   and M.dictatorID NOT IN (Select A.UserID FROM SEcureWEb.dbo.AssignDic A INNER JOIN SecureWeb.dbo.GrpDictators G ON G.GrpDicID = A.GrpDicID where G.AccID = '" & DRRec1("AccountID").ToString & "')   "
                                'Response.Write(strQuery)
                                    Dim SQLCmdChk1 As New SqlCommand(strQuery, New SqlConnection(strConn))
                                    Try
                                        SQLCmdChk1.Connection.Open()
                                ...
                                End Try
                                If ErrFound = False Then
                                    strQuery = "Select * from SecureWeb.dbo.GrpDictators A LEFT OUTER JOIN Secureweb.dbo.BillDetails B ON A.AccID= B.AccountID and A.GrpDicID = B.SubActID where A.AccID = '" & DRRec1("AccountID").ToString & "' and B.SubActID IS NULL  "
                                    ' Response.Write(strQuery)
                                    Dim SQLCmdChk2 As New SqlCommand(strQuery, New SqlConnection(strConn))
                                    Try
                                        SQLCmdChk2.Connection.Open()
                                                                ...
                                                                If DRRec4.HasRows = True Then
                                                                    ...
                                                                    If DRRec4.Read Then
                                                                        ...
                                                                        Else
                                                                            DefaultLC = False
                                                                        End If
                                                                        strQuery = "Select distinct T.*, M.Type,M.Location,CASE WHEN M.Priority = 1 AND (DATEDIFF(MINUTE, M.DateCreated, ISNULL(M.DateModified, GETDATE()))/60) < M.TAT THEN 'True' ELSE 'False' END as Priority,M.DictatorID,M.Jobnumber from SecureWeb.dbo.tblTranscriptionClientMain M INNER JOIN SEcureWEb.dbo.AssignDic A ON A.UserID = M.dictatorID  Left Outer Join tblTransLC T ON T.TranscriptionID = M.TranscriptionID where T.TranscriptionID is not NULL and A.AccID = '" & DRRec1("AccountID").ToString & "'   and datediff(day,M.datemodified, '" & edate & "') between 0 and datediff(day,'" & sdate & "','" & edate & "')   and A.GrpDicID = '" & SubActID & "' order by JobNumber"
            ...
            For i As Integer = 0 To UBound(splt)
                ...
                If Not String.IsNullOrEmpty(BillAccID) Then
                    Try
                        ...
                        strQuery = "Select * from SEcureWeb.dbo.tblBillAccounts where billAccID = '" & BillAccID & "' "
                        ...
                        strQuery = "DELETE FROM [Secureweb].[dbo].[tblBillGeneratedDetails] WHERE [AccountID]= '" & AccountID & "' and [BillCycle] = '" & BillCycle & "' and [BillMonth] ='" & BillMonth & "' and [BillYear] ='" & BillYear & "' "
                        cmdUp.CommandText = strQuery
                        cmdUp.CommandType = CommandType.Text
                        cmdUp.ExecuteNonQuery()
                        strQuery = "INSERT INTO [Secureweb].[dbo].[tblBillGeneratedDetails]([AccountID] ,[BillCycle] ,[BillMonth] ,[BillYear]  ,[Status]  ,[ErrMessage],[DateUpdate])  VALUES ('" & AccountID & "', '" & BillCycle & "', '" & BillMonth & "', '" & BillYear & "', 'Removed', '', '" & Now & "') "
                        cmdUp.CommandText = strQuery
                        cmdUp.CommandType = CommandType.Text
                        cmdUp.ExecuteNonQuery()
    ...
    Protected Sub InsertLog(ByVal AccountID As String, ByVal BillCycle As Integer, ByVal BillMonth As Integer, ByVal BillYear As Integer, ByVal Status As String, ByVal ErrMessage As String)
        ...
        Try
            ...
            cmdUp.CommandType = CommandType.Text
            cmdUp.ExecuteNonQuery()
            strQuery = "INSERT INTO [Secureweb].[dbo].[tblBillGeneratedDetails]([AccountID] ,[BillCycle] ,[BillMonth] ,[BillYear]  ,[Status]  ,[ErrMessage],[DateUpdate])  VALUES ('" & AccountID & "', '" & BillCycle & "', '" & BillMonth & "', '" & BillYear & "', '" & Status & "', '" & ErrMessage & "', '" & Now & "') "
            cmdUp.CommandText = strQuery
            cmdUp.CommandType = CommandType.Text
            cmdUp.ExecuteNonQuery()
            MyTransAttr.Commit()
