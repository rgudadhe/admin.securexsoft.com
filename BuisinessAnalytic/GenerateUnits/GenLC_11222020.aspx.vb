    ...
    Protected Sub BtnSubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
        ...
        If DLCycle.SelectedValue = "1" Then
            ...
            If DLAct.SelectedValue = "All" Then
                strQuery = "Select A.*, 0 As IsAccFaxBilling, 0 AS AccFaxCharges, 0 As IsAccVPNBilling, 0 AS AccVPNCharges, 0 As IsAccCutPasteBilling, 0 AS AccCutPasteBilling , 0 AS FaxPageCount  from ETS.dbo.tblaccounts A where (Isdeleted Is NULL OR Isdeleted = 0) and  contractorid ='" & Session("contractorid").ToString & "'  and Cycle='" & strCycle & "' order by Accountname"
            Else
        ...
        Else
            strCycle = False
            If DLAct.SelectedValue = "All" Then
                strQuery = "Select A.*, Isnull(A.IsFaxBilling,0) As IsAccFaxBilling, ISNULL(A.FaxCharges,0) AS AccFaxCharges, Isnull(A.IsVPNBilling,0) As IsAccVPNBilling, ISNULL(A.VPNCharges,0) AS AccVPNCharges, Isnull(A.IsCutPasteBilling,0) As IsAccCutPasteBilling, ISNULL(A.CutPasteBilling,0) AS AccCutPasteBilling , ISNULL(F.FaxPageCount, 0) AS FaxPageCount from ETS.dbo.tblaccounts A  LEFT OUTER JOIN (select AccountID, SUM(r.documentpagecount) AS FaxPageCount from tbloutbound O INNER JOIN  tblJobDeliveryResult R ON R.JobID =O.JOBID and R.RecordID = O.RecordID INNER JOIN secureweb.dbo.tbltranscriptionclient C ON C.TranscriptionID = O.JOBID WHERE (O.StatusID =1 or O.StatusID =5) and O.lastattempttime > '" & C1CurrSDate & "' and o.lastattempttime < '" & C2CurrEDate.AddDays(1) & "' group by AccountID ) F ON F.AccountID=A.AccountID where A.contractorid ='" & Session("contractorid").ToString & "' and (A.Isdeleted Is NULL OR A.Isdeleted = 0)    order by A.Accountname"
            Else
