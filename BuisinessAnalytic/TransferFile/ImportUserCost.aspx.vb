...
Partial Class Billing_FileImport_ImportLines
    ...
    Protected Sub InsertRec(ByVal UserID As String, ByVal target As String, ByVal salary As String, ByVal currency As String)
        ...
        Try
            oConn.Open()
            strQuery = "INSERT INTO [ADMINETS].[dbo].[tblUsersTargets] ([userid],[target],[salary],[currency],[updatedate] ) VALUES "
            strQuery = strQuery & " ('" & UserID & "' ,'" & target & "' ,'" & salary & "' ,'" & currency & "' ,'" & Now() & "'   )"
    ...
    Protected Sub UpdateRec(ByVal UserID As String, ByVal target As String, ByVal salary As String, ByVal currency As String)
        ...
        Try
            oConn.Open()
            strQuery = "UPDATE [ADMINETS].[dbo].[tblUsersTargets]  SET target = '" & target & "' ,salary = '" & salary & "' ,currency =  '" & currency & "', updatedate = '" & Now & "' WHERE userID = '" & UserID & "' "
                ...
                If UserName <> "" Then
                    ...
                    If DRRec2.HasRows Then
                        If DRRec2.Read Then
                            ...
                            strQuery = "Select * from [ADMINETS].[dbo].[tblUsersTargets] where  userID = '" & UserID & "'  "
                            ' Response.Write(strQuery)
