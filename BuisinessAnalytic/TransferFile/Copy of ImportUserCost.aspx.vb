...
Partial Class Billing_FileImport_ImportLines
    ...
    Protected Sub InsertRec(ByVal UserID As String, ByVal target As String, ByVal salary As String, ByVal currency As String)
        ...
        Try
            oConn.Open()
            strQuery = "INSERT INTO [ETS].[dbo].[tblUsersTargetsMonthly] ([userid],[target],[salary],[currency],[procmonth],[procyear],[updatedate] ) VALUES "
    ...
    Protected Sub UpdateRec(ByVal UserID As String, ByVal target As String, ByVal salary As String, ByVal currency As String)
        ...
        Try
            oConn.Open()
            strQuery = "UPDATE [ETS].[dbo].[tblUsersTargetsMonthly]  SET target = '" & target & "' ,salary = '" & salary & "' ,currency =  '" & currency & "', updatedate = '" & Now & "' WHERE userID = '" & UserID & "' and ProcMonth  = '" & DLMonth.SelectedValue & "' and ProcYear  = '" & DLYear.SelectedValue & "'"
        ...
        If DRRec1.HasRows Then
            While (DRRec1.Read)
                ...
                If UserName <> "" Then
                    ...
                    strQuery = "Select userid, FirstName + ' ' + LastName as Name from ets.dbo.tblusers where contractorid='" & Session("contractorid").ToString & "' and username = '" & UserName & "' "
                    '  Response.Write(strQuery)
                    ...
                    If DRRec2.HasRows Then
                        If DRRec2.Read Then
                            ...
                            strQuery = "Select * from [ETS].[dbo].[tblUsersTargetsMonthly] where  userID = '" & UserID & "' and ProcMonth  = '" & DLMonth.SelectedValue & "' and ProcYear  = '" & DLYear.SelectedValue & "' "
                            ' Response.Write(strQuery)
