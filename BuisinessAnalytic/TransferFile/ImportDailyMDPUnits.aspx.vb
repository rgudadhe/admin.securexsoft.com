...
Partial Class Login_CreateUser
    ...
    Public Function InsertRec(ByVal username As String, ByVal JobNumber As String, ByVal UserLevel As String, ByVal Lines As String, ByVal PostDate As String) As Boolean
        ...
        Try
            oConn.Open()
            strQuery = "INSERT INTO AdminETS.dbo.tblUserPlatLines (PlatActID,username, JobNumber, UserLevel, Lines,  PostDate, updatedate) VALUES "
            strQuery = strQuery & " ('0E8D4B01-188A-4047-8816-91406C6FC275','" & username & "' ,'" & JobNumber & "' ,'" & UserLevel & "' ,'" & Lines & "'  ,'" & PostDate & "', '" & Now() & "')"
    ...
    Protected Sub ShowDetails(ByVal username As String, ByVal JobNumber As String, ByVal UserLevel As String, ByVal Lines As String, ByVal PostDate As String, ByVal Status As String)
                ...
                If DRRec1.HasRows Then
                    While (DRRec1.Read)
                        If DRRec1(0).ToString.Replace("'", "''") <> "" And DRRec1(1).ToString.Replace("'", "''") <> "" Then
                            ...
                            If MTID = QAID Then
                                UserLevel = 2
                                If InsertRec(UserMT, JobNumber, UserLevel, Lines, PostDate) = True Then
                                    ShowDetails(UserMT, JobNumber, UserLevel, Lines, PostDate, "Imported")
                                End If
                            Else
                                If GetSXFID(DRRec1(3).ToString.Replace("'", "''"), "QAID") <> "" Then

                                    UserLevel = 1
                                    If InsertRec(UserMT, JobNumber, UserLevel, Lines, PostDate) = True Then
                                        ShowDetails(UserMT, JobNumber, UserLevel, Lines, PostDate, "Imported")
                                    End If
                                    UserQA = GetSXFID(DRRec1(3).ToString.Replace("'", "''"), "QAID")
                                    UserLevel = 3
                                    If InsertRec(UserQA, JobNumber, UserLevel, Lines, PostDate) = True Then
                                        ShowDetails(UserQA, JobNumber, UserLevel, Lines, PostDate, "Imported")
                                    End If
                                ElseIf GetSXFID(DRRec1(3).ToString.Replace("'", "''"), "QABID") <> "" Then
                                    UserLevel = 4
                                    If InsertRec(UserMT, JobNumber, UserLevel, Lines, PostDate) = True Then
                                        ShowDetails(UserMT, JobNumber, UserLevel, Lines, PostDate, "Imported")
                                    End If
                                    UserQA = GetSXFID(DRRec1(3).ToString.Replace("'", "''"), "QABID")
                                    UserLevel = 6
                                    If InsertRec(UserQA, JobNumber, UserLevel, Lines, PostDate) = True Then
                                        ShowDetails(UserQA, JobNumber, UserLevel, Lines, PostDate, "Imported")
                                    End If
