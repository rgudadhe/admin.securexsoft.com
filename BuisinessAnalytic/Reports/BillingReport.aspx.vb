...
Partial Class MIS_DailyMins
    ...
    Protected Sub ShowActDetails(ByVal StartDate As Date, ByVal EndDate As Date)
        ...
        strQuery = "select A.AccountName, sum(L.Amount) AS LINES, SUM(DATEDIFF(S, 0,DURATION)/60) AS MINS, Count(M.TranscriptionID) AS NumJobs from AdminSecureweb.dbo.tbldictationdetails M, tblBillingLines L, tblAccounts A "
        strQuery = strQuery & " WHERE  M.TranscriptionID = L.TranscriptionID and A.AccountID=M.AccountID and A.contractorID = '" & Session("contractorID").ToString & "'  and  datediff(day,M.datemodified, '" & EndDate & "') between 0 and " & daydiff & "    "
        ...
        Try
            ...
            If DRRecT.HasRows Then
                While DRRecT.Read
                    ...
                    Cell4.Text = FormatNumber(DRRecT("Lines").ToString, 0)
                ...
                tCell2.Text = numjobs
                tCell3.Text = mins
                tCell4.Text = FormatNumber(lines, 0)
