...
Partial Class ViewUnitsTrend
    ...
    Protected Sub ShowActDetails()
        ...
        '    DayDiff = DateDiff(DateInterval.Day, startdate, EndDate)
        'End If
        startdate1 = DLQuarter1.SelectedValue & "/1/" & DLYear1.SelectedValue
        startdate2 = DLQuarter2.SelectedValue & "/1/" & DLYear2.SelectedValue
        EndDate2 = startdate2.AddMonths(3).AddDays(-1)
        DayDiff = DateDiff(DateInterval.Day, startdate1, EndDate2)
        MonthDiff = DateDiff(DateInterval.Quarter, startdate1, EndDate2)
        Dim ParmValues As String = String.Empty
        ParmValues = "[" & startdate1.ToString("MM/dd/yyyy") & "]"
        Dim incr As Integer
        For I As Integer = 1 To MonthDiff
            incr = incr + 2
            ParmValues = ParmValues & ",[" & startdate1.AddMonths(I + incr).ToString("MM/dd/yyyy") & "]"
        Next

        Dim CompDate As Date
        If CheckBox1.Checked = True Then
            CompDate = DLQuarter.SelectedValue & "/1/" & DLYear.SelectedValue
            ParmValues = ParmValues & ",[CompMonth]"
            For incr1 As Integer = DLQuarter.SelectedValue To DLQuarter.SelectedValue + 2
                If incr1 = DLQuarter.SelectedValue Then
                    CompMonth = "'" & incr1 & "'"
                Else
                    CompMonth = CompMonth & "," & "'" & incr1 & "'"
                End If
            Next
            'CompMonth = DLQuarter.SelectedValue
            CompYear = DLYear.SelectedValue
        End If
        'Response.Write(CompMonth)
        'Response.End()
        Dim DTSet2 As System.Data.DataSet = objrep.GetQuarterlyTrends(New Guid(Session("ContractorId").ToString), DayDiff, EndDate2, Nothing, ParmValues, CheckBox1.Checked, CompMonth, CompYear, DLTrend.SelectedValue)
            ...
            If DTSet2.Tables(0).Rows.Count > 0 Then
                ...
                For Each dc As DataColumn In DTSet2.Tables(0).Columns
                    ...
                    If IsDate(dc.ColumnName) Then
                        dt = dc.ColumnName
                        dc.ColumnName = "Q" & (Month(dt) + 2) / 3 & " " & dt.ToString("yyyy")
                        'Response.Write(dc.ColumnName)
                    End If

                Next
                If CheckBox1.Checked = True Then
                    DTSet2.Tables(0).Columns((DTSet2.Tables(0).Columns.Count - 1)).ColumnName = "- Q" & (Month(CompDate) + 2) / 3 & " " & CompDate.ToString("yyyy")
                End If
    ...
    Protected Sub AttachColumn()
        For Each objRow As GridViewRow In MyDataGrid.Rows
            ...
            Else
                'objRow.Cells(1).Text = "<a href='ViewCustomerReportByDict.aspx?showdic=Yes&DLAct=" & MyDataGrid.DataKeys(objRow.RowIndex).Value.ToString & "&TXTSDate=" & TXTSDate.Text & "&TXTEDate=" & TXTEDate.Text & "'>" & objRow.Cells(1).Text & "</a>"
            End If
        Next
        'Dim dt As Date
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        filename = "Customer Quarterly Trend" & Month(Now) & Day(Now) & Year(Now) & ".xls"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            Dim Quart1 As Integer
            Dim Quart2 As Integer
            Dim year1 As Integer
            Dim year2 As Integer
            If Now.Month = 1 Or Now.Month = 2 Or Now.Month = 3 Then
                Quart1 = 10
                year1 = (Now.AddYears(-1).Year)
                Quart2 = 1
                year2 = Now.Year
            ElseIf Now.Month = 4 Or Now.Month = 5 Or Now.Month = 6 Then
                Quart1 = 4
                year1 = Now.Year
                Quart2 = 1
                year2 = Now.Year
            ElseIf Now.Month = 7 Or Now.Month = 8 Or Now.Month = 9 Then
                Quart1 = 7
                year1 = Now.Year
                Quart2 = 4
                year2 = Now.Year
            ElseIf Now.Month = 10 Or Now.Month = 11 Or Now.Month = 12 Then
                Quart1 = 10
                year1 = Now.Year
                Quart2 = 7
                year2 = Now.Year
            End If
            DLQuarter1.SelectedIndex = DLQuarter1.Items.IndexOf(DLQuarter1.Items.FindByValue(Quart1))
            DLYear1.SelectedIndex = DLYear1.Items.IndexOf(DLYear1.Items.FindByValue(year1))
            DLQuarter2.SelectedIndex = DLQuarter2.Items.IndexOf(DLQuarter2.Items.FindByValue(Quart2))
            DLYear2.SelectedIndex = DLYear2.Items.IndexOf(DLYear2.Items.FindByValue(year2))
        End If
    End Sub
End Class
