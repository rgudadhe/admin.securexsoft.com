...
Partial Class MIS_DailyMins
        ...
        Try
            ...
            If DRRec.HasRows Then
                While DRRec.Read
                    ...
                    Cell2.Text = FormatNumber(DRRec("Billunits").ToString, 2)
    ...
    Protected Sub DictDetails(ByVal sBillAccID As String, ByVal sSubActID As String, ByVal sPriority As Boolean, ByVal sMode As String, ByVal sSubActName As String)
        Try
            ...
            StrQuery = "Select M.DictatorID, P.dname, Sum(B.Unit) as BillUnits, Sum(B.Amount) As Amount , count(B.TranscriptionID) As RecCount "
            StrQuery = StrQuery & " from tblBillingLines B "
            StrQuery = StrQuery & "  LEFT OUTER JOIN (Select * from AdminSecureweb.dbo.tblTranscriptionClientMain) M"
            StrQuery = StrQuery & " on B.TranscriptionID = M.TranscriptionID"
            StrQuery = StrQuery & "  INNER JOIN (Select firstname + ' ' + lastname as dname, PhysicianID  from tblPhysicians) P"
            ...
            Try
                ...
                If DRRec.HasRows Then
                    While DRRec.Read
                        ...
                        Row1.Cells.Add(Cell3)
                        tblDict.Rows.Add(Row1)
                        TotUnits += FormatNumber(DRRec("Billunits").ToString, 2)
                        TotRecCount += CInt(DRRec("RecCount"))
                        TotAmt += DRRec("Amount")
                        'Response.Write(DRRec("Amount") & "#")
                    End While
            ...
            tCell2.Text = FormatNumber(TotUnits, 2)
            tCell3.Text = TotRecCount
            tCell4.Text = FormatNumber(TotAmt, 2)
            tRow1.Cells.Add(tCell1)
            tRow1.Cells.Add(tCell2)
    ...
    Protected Sub RepDetails(ByVal sBillAccID As String, ByVal sSubActID As String, ByVal sPriority As Boolean, ByVal sMode As String, ByVal sDictID As String)
        Try
            ...
            TotUnits = 0
            strConn = System.Configuration.ConfigurationManager.AppSettings("ETSCon")
            StrQuery = "Select P.dname, JobNumber, CustJobID, M.SubmitDate,M.dateModified, B.Unit, B.Amount,B.Rate, TA.Patient, TA.MedRN, TA.DtOfServ, B.Priority, T1.TemplateName  "
            StrQuery = StrQuery & " from tblBillingLines B "
            StrQuery = StrQuery & "  LEFT OUTER JOIN (Select * from AdminSecureweb.dbo.tblTranscriptionClientMain) M"
            StrQuery = StrQuery & " on B.TranscriptionID = M.TranscriptionID  LEFT OUTER JOIN (Select TranscriptionID, PFirstName + ' ' + PLastName as Patient, MedRN, DtofServ from secureweb.dbo.tbltransattributes  UNION Select  TranscriptionID, PFirstName + ' ' + PLastName as Patient, MedRN, DtofServ from secureweb.dbo.tbltransattributes_Arch) TA  on B.TranscriptionID = TA.TranscriptionID "
            StrQuery = StrQuery & "  LEFT OUTER JOIN (Select * from AdminETS.dbo.tbltemplates) T1"
            ...
            Try
                ...
                If DRRec.HasRows Then
                    While DRRec.Read
                        ...
                        Cell5.Text = FormatNumber(DRRec("Unit").ToString, 2)
                        ...
                        Cell12.Text = DRRec("MedRN").ToString
                        If IsDate(DRRec("DtofServ").ToString) Then
                            Dim DOS As Date = DRRec("DtofServ").ToString
                            Cell13.Text = DOS.ToString("MM/dd/yyyy")
                        Else
                            Cell13.Text = ""
                        End If
                        ...
                        TotUnits += FormatNumber(DRRec("Unit").ToString, 2)
