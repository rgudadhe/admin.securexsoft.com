...
Partial Class ViewUnitsTrend
    ...
    Protected Sub ShowActDetails()
        ...
        Dim objrep As New ETS.BL.BusinessAnalytics
            ...
            Dim DTSet2 As System.Data.DataSet = objrep.GetEMPMTDProductivityReport(New Guid(Session("contractorID").ToString), CurrSDate, InpDate, Session("WorkGroupId").ToString)

            ' Response.Write(CurrSDate & "#" & InpDate & "#" & Session("ContractorId").ToString)
            If DTSet2.Tables.Count > 0 Then
                ...
                If DTSet2.Tables(0).Rows.Count > 0 Then
                    ...
                    For I As Integer = 0 To DTSet2.Tables(0).Rows.Count - 1
                        ClmCount = 0
                        For j As Integer = 7 To DTSet2.Tables(0).Columns.Count - 1
                            'ClmCount += 1
                            If I = 0 Then
                    ...
                    'Response.Write(DTSet2.Tables(0).Rows(0).Item(DTSet2.Tables(0).Columns.Count - 2).ToString)
                    DTSet2.Tables(0).Rows(0).Item(DTSet2.Tables(0).Columns.Count - 1) = FormatNumber((DTSet2.Tables(0).Rows(0).Item(DTSet2.Tables(0).Columns.Count - 8).ToString / DTSet2.Tables(0).Rows(0).Item(DTSet2.Tables(0).Columns.Count - 2).ToString) * 100, 0)
                    DTSet2.Tables(0).Rows(0).Item(DTSet2.Tables(0).Columns.Count - 6) = FormatNumber((DTSet2.Tables(0).Rows(0).Item(DTSet2.Tables(0).Columns.Count - 8).ToString / DTSet2.Tables(0).Rows(0).Item(DTSet2.Tables(0).Columns.Count - 7).ToString) * 100, 0)
    ...
    Protected Sub AttachColumn()
        Dim PerColumn1 As Integer = MyDataGrid.Columns.Count - 2
        Dim PerColumn2 As Integer = MyDataGrid.Columns.Count - 7
        Dim PerColumn3 As Integer = MyDataGrid.Columns.Count - 1
        For Each objRow As GridViewRow In MyDataGrid.Rows
            If objRow.RowIndex = 0 Then
                ...
                objRow.Cells(PerColumn1).Text = String.Empty
                objRow.Cells(PerColumn2).Text = String.Empty
                objRow.Cells(PerColumn3).Text = String.Empty
            Else
                'objRow.Font.Bold = True
                ...
                objRow.Cells(PerColumn1).Text = objRow.Cells(PerColumn1).Text & " %"
                objRow.Cells(PerColumn2).Text = objRow.Cells(PerColumn2).Text & " %"
                objRow.Cells(PerColumn3).Text = objRow.Cells(PerColumn3).Text & " %"
            End If
        Next
        'Dim dt As Date
