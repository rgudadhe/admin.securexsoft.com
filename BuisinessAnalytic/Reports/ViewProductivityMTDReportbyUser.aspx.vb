...
Partial Class ViewUnitsTrend
    Inherits BasePage
    Protected CurrSDate As Date
    Protected InpDate As Date
    Protected Sub ShowActDetails()
        ...
        'Response.Write(InpDate & Request("userid"))

        DayDiff = DateDiff(DateInterval.Day, CurrSDate, InpDate)

        Dim DTSet2 As System.Data.DataSet = objrep.GetEMPMTDDailyProductivityReportbyUser(New Guid(Request("userid")), CurrSDate, InpDate)
        ...
        If DTSet2.Tables.Count > 0 Then
            If DTSet2.Tables(0).Rows.Count > 0 Then
                ...
                For I As Integer = 0 To DTSet2.Tables(0).Rows.Count - 1
                    ClmCount = 0
                    For j As Integer = 1 To DTSet2.Tables(0).Columns.Count - 1
                        'ClmCount += 1
                        If I = 0 Then
                ...
                DTRow(0) = "Total"
                For j As Integer = 1 To DTSet2.Tables(0).Columns.Count - 4
                    'If j = DTSet2.Tables(0).Columns.Count - 1 Then
                    '    DTRow(j) = FormatNumber((ColTotal(j - 2) / ColTotal(j - 1)) * 100, 0)
                    'Else
    ...
    Protected Sub AttachColumn()
        Dim PerColumn As Integer = MyDataGrid.Columns.Count - 1
        ...
        For Each objRow As GridViewRow In MyDataGrid.Rows
            If objRow.RowIndex = 0 Then
                ...
                objRow.Cells(0).Text = "<a href='ViewProductivityJobWisebyUser.aspx?ShowAct=Yes&userid=" & Request("userid") & "&Enddate=" & InpDate & "&StartDate=" & CurrSDate & "'>" & objRow.Cells(0).Text & "</a>"
                objRow.Cells(0).BackColor = Drawing.Color.White
            Else
                objRow.Cells(0).Text = "<a href='ViewProductivityJobWisebyUser.aspx?ShowAct=Yes&userid=" & Request("userid") & "&Enddate=" & objRow.Cells(0).Text & "&StartDate=" & objRow.Cells(0).Text & "'>" & objRow.Cells(0).Text & "</a>"
                objRow.Cells(PerColumn).Text = objRow.Cells(PerColumn).Text & " %"
