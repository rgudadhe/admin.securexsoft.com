...
Partial Class ViewUnitsTrend
    Inherits BasePage
    Protected sMonth As Integer
    Protected sYear As Integer
    Protected Sub ShowActDetails()
        Try
            ...
            '    DayDiff = DateDiff(DateInterval.Day, startdate, EndDate)
            'End If
            sMonth = DLMonth1.SelectedValue
            sYear = DLYear1.SelectedValue
            startdate1 = DLMonth1.SelectedValue & "/1/" & DLYear1.SelectedValue
            EndDate2 = startdate1.AddMonths(1).AddDays(-1)
            ...
            For I As Integer = 1 To DayDiff + 1
                If I = 1 Then
                    ParmValues = "[" & I & "]"
                Else
                    ParmValues = ParmValues & ",[" & I & "]"
                End If

            Next
            'Response.Write(Session("ContractorId").ToString & "#" & DayDiff & "#" & EndDate2 & "#" & Nothing & "#" & ParmValues & "#" & DLTrend.SelectedValue & "#" & Session("WorkGroupId").ToString)
            'Response.Write(ParmValues)
            Dim DTSet2 As System.Data.DataSet = objrep.GetDailyProdTrends(New Guid(Session("ContractorId").ToString), DayDiff, EndDate2, Nothing, ParmValues, DLTrend.SelectedValue, Session("WorkGroupId").ToString)
    ...
    Protected Sub AttachColumn()
        For Each objRow As GridViewRow In MyDataGrid.Rows
            ...
            Else
                For I As Integer = 5 To MyDataGrid.Columns.Count - 1
                    If objRow.Cells(I).Text > 0 Then
                        objRow.Cells(I).Text = "<a href='ViewProductivityJobWisebyUser.aspx?ShowAct=Yes&userid=" & objRow.Cells(0).Text & "&Enddate=" & sMonth & "/" & MyDataGrid.Columns(I).HeaderText & "/" & sYear & "&StartDate=" & sMonth & "/" & MyDataGrid.Columns(I).HeaderText & "/" & sYear & "'>" & objRow.Cells(I).Text & "</a>"
                    End If
                Next
            End If
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        filename = "Customer Daily Trend" & Month(Now) & Day(Now) & Year(Now) & ".xls"
