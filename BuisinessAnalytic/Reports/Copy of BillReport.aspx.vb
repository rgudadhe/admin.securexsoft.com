...
Partial Class MIS_DailyMins
    ...
    Protected Sub ShowActDetails()
        ...
        TBillOtherAmt = 0
        TBillTotAmount = 0
        strQuery = "Select A.Accountid, A.Indirect, IsNull(V1.OthCharges,0) as ActOthCharges1, IsNull(V2.OthCharges,0) as ActOthCharges2, IsNull(M.minCharges,0) as MinCharges, BA.Mode, A.AccountName as Description, A.BillActNumber, C.priority, C.Description as CateDescr, BA.MinBilling, BA.BillAccID, BA.Posted, I.Amount as InvAmount, I.trackID, I.InvCode, I.InvRecFound, BA.BillCycle, BA.Cycle    "
        strQuery = strQuery & " from tblaccounts A "
        strQuery = strQuery & " INNER JOIN (Select * from SecureWeb.dbo.tblBillAccounts) BA"
        ...
        strQuery = strQuery & " LEFT OUTER JOIN (Select TrackId, InvCode, AccID, Amount, 'Yes' as InvRecFound from SecureWeb.dbo.InvupData where InvType ='Invoice' and BillMonth='" & DLMonth.SelectedValue & "' and Billyear='" & DLYear.SelectedValue & "' and BillCycle='Cycle" & DLCycle.SelectedValue & "' ) I "
        strQuery = strQuery & " on I.AccID = BA.AccountID"
        strQuery = strQuery & " LEFT OUTER JOIN (Select AccountID, sum(totamount) as OthCharges from SecureWeb.DBO.tblInvItemDet where Mode = 'VAS' and Servicedate >=  '" & C1SDate & "' and Servicedate < '" & C2SDate & "'  Group By AccountID) V1  on V1.accountid = A.Accountid"
        strQuery = strQuery & " LEFT OUTER JOIN (Select AccountID, sum(totamount) as OthCharges from SecureWeb.DBO.tblInvItemDet where Mode = 'VAS' and Servicedate between  '" & C1EDate & "' and '" & C2EDate & "'  Group By AccountID) V2  on V2.accountid = A.Accountid"
        strQuery = strQuery & " LEFT OUTER JOIN (Select AccountID, sum(totamount) as minCharges from SecureWeb.DBO.tblInvItemDet where ItemID='25c7d577-967e-48ab-a62d-87fb6f420be1' and Mode = 'VAS' and Servicedate between  '" & C1SDate & "' and '" & C2EDate & "'  Group By AccountID) M  on M.accountid = A.Accountid"
        If MonthSel = True Then
            strQuery = strQuery & "  where BA.BillMonth = '" & DLMonth.SelectedValue & "' and BA.BillYear='" & DLYear.SelectedValue & "' and (A.Indirect IS NULL or A.Indirect = 'False' ) and A.contractorid = '" & Session("contractorid").ToString & "'  Order by Priority, A.BillActNumber"
        Else
            strQuery = strQuery & "  where BA.BillMonth = '" & DLMonth.SelectedValue & "' and BA.BillYear='" & DLYear.SelectedValue & "' and BillCycle = '" & DLCycle.SelectedValue & "' and (A.Indirect IS NULL or A.Indirect = 'False' )  and A.contractorid = '" & Session("contractorid").ToString & "'  Order by Priority, A.BillActNumber "
        End If
        ...
        'strQuery = strQuery & " from tblaccounts A "
        'strQuery = strQuery & " LEFT OUTER JOIN (Select * from SecureWeb.dbo.tblBillAccounts) BA"
        'strQuery = strQuery & " on BA.AccountID = A.AccountID"
        'strQuery = strQuery & " LEFT OUTER JOIN (Select * from tblActCategories) C"
        'strQuery = strQuery & " on C.category = A.Category"
        'strQuery = strQuery & " LEFT OUTER JOIN (Select TrackId, InvCode, AccID, Amount, 'Yes' as InvRecFound from SecureWeb.dbo.InvupData where InvType ='Invoice' and BillMonth='" & DLMonth.SelectedValue & "' and Billyear='" & DLYear.SelectedValue & "' and BillCycle='Cycle" & DLCycle.SelectedValue & "' ) I "
                ...
                While DRRec.Read
                    ...
                    Else
                        ...
                        If Mode = "DC" Or Mode = "TW" Or Mode = "LC" Or Mode = "TT" Or Mode = "DV" Then
                            strQuery = "Select  B.SubActID, B.Rate, L.MethodName, isnull(T1.Lines,0) AS BillUnits, isnull(T2.Lines,0) AS BillSUnits,  isnull(T1.stdLines,0) AS stdLines, isnull(T2.stdLines,0) AS stdSLines, isnull(T1.Amt,0) AS BillAmt, isnull(T2.Amt,0) AS BillSAmt     "
                            strQuery = strQuery & " from SecureWeb.DBO.tblAccBillDetails B"
                            strQuery = strQuery & " LEFT OUTER JOIN (Select TrackID, MethodName from ETS.DBO.tblLCMethodology ) L"
                            ...
                            If DRRec1.HasRows Then
                                While (DRRec1.Read)
                                    If DRRec1("MethodName").ToString = "PerDictator" Then
                                        strQuery = "Select count(*) as NumDict from SecureWeb.DBO.AssignDic where GrpDicID = '" & DRRec1("SubActID").ToString & "'  "
                                        'strQuery = "Select count(*) as NumDict from (Select M.DictatorID as Numdict from ETS.dbo.tbltranscriptionMain M INNER Join tblBillingLines T ON T.TranscriptionID = M.TranscriptionID where  T.BillAccID = '" & BillAccID & "' and T.SubActID = '" & DRRec1("SubActID").ToString & "' Group by M.dictatorID)T1 "
                                ...
                                Else
                                    If minbilling > BillAmt Then
                                        ...
                                        RemBalance = minbilling - BillAmt


                                        If Not FormatNumber(RemBalance, 2) = FormatNumber(DRRec("MinCharges").ToString, 2) And (Not DLCycle.SelectedValue = 3) Then
                                            Response.Write(FormatNumber(RemBalance, 2) & "#" & FormatNumber(DRRec("MinCharges").ToString, 2))
    ...
    Function InsRecord(ByVal AccID As String, ByVal ItemID As String, ByVal Descr As String, ByVal Quantity As Integer, ByVal amount As Double, ByVal Totamount As Double, ByVal ServiceDate As String) As Boolean
        Try
            ...
            Dim strQuery As String
            strConn = System.Configuration.ConfigurationManager.AppSettings("ETSCon")
            strQuery = "Insert Into Secureweb.Dbo.tblinvItemdet (AccountID,InvoiceID, itemid, Descr, quantity, Amount,totamount,mode,servicedate, dateupdate) Values ('" & AccID & "', '11111111-1111-1111-1111-111111111111', '" & ItemID & "', '" & Descr & "', '" & Quantity & "', convert(money," & amount & "),  convert(money," & Totamount & "), 'VAS', '" & ServiceDate & "', '" & Now & "')"
            '            Response.Write(strQuery)
