...
Partial Class DailyTeamReport
    ...
    Protected Sub ShowActDetails()
        ...
        strQuery = strQuery & " SELECT U.FIRSTNAME + ' ' + U.LASTNAME AS UNAME, U.USERNAME, U.TRGLINES, U.ProdMode AS DNAME, ISNULL(A.ACCOUNTNAME, 'Direct Accounts') AS ACCOUNTNAME "
        ...
        strQuery = strQuery & " ,ISNULL(TS6.TSLINES,0) AS MTBLINES"
        strQuery = strQuery & " ,ISNULL(TS7.TSLINES,0) AS QABSELINES"
        strQuery = strQuery & " ,ISNULL(TS8.TSLINES,0) AS TMTLINES"
        strQuery = strQuery & " ,ISNULL(TS9.TSLINES,0) AS TMTPLINES"
        strQuery = strQuery & " ,ISNULL(TS10.TSLINES,0) AS TQALINES"
        strQuery = strQuery & " ,ISNULL(TS11.TSLINES,0) AS TBBLINES"
        strQuery = strQuery & " ,ISNULL(TS12.TSLINES,0) AS TPPQALINES"
        strQuery = strQuery & " ,ISNULL(TS13.TSLINES,0) AS TMTBLINES"
        strQuery = strQuery & " ,ISNULL(TS14.TSLINES,0) AS TQABSELINES"
        strQuery = strQuery & " FROM TBLUSERS U"
        strQuery = strQuery & " LEFT JOIN TBLDEPTDESIGNATIONS D ON U.DESIGNATIONID = D.DESIGNATIONID"
        'strQuery = strQuery & " LEFT JOIN TBLUSERSCATEGORY C ON U.CATEGORYID = C.CATEGORYID"
        ...
        strQuery = strQuery & " LEFT JOIN (SELECT MTID, SUM(STDBODYLC) AS TSLINES FROM AdminSecureweb.dbo.TBLDICTATIONDETAILS "
        strQuery = strQuery & " WHERE DATEMODIFIED BETWEEN '" & InpDate & "' AND '" & CurrEDate & "' GROUP BY MTID ) AS TS1 "
        strQuery = strQuery & " ON TS1.MTID = U.USERID"
        strQuery = strQuery & " LEFT JOIN (SELECT MTPLUSID, SUM(STDBODYLC) AS TSLINES FROM AdminSecureweb.dbo.TBLDICTATIONDETAILS "
        strQuery = strQuery & " WHERE DATEMODIFIED BETWEEN '" & InpDate & "' AND '" & CurrEDate & "' GROUP BY MTPLUSID ) AS TS2 "
        strQuery = strQuery & " ON TS2.MTPLUSID = U.USERID"
        strQuery = strQuery & " LEFT JOIN (SELECT QAID, SUM(STDBODYLC) AS TSLINES FROM AdminSecureweb.dbo.TBLDICTATIONDETAILS "
        strQuery = strQuery & " WHERE DATEMODIFIED BETWEEN '" & InpDate & "' AND '" & CurrEDate & "' GROUP BY QAID ) AS TS3 "
        strQuery = strQuery & " ON TS3.QAID = U.USERID"
        strQuery = strQuery & " LEFT JOIN (SELECT QABID, SUM(STDBODYLC) AS TSLINES FROM AdminSecureweb.dbo.TBLDICTATIONDETAILS "
        strQuery = strQuery & " WHERE DATEMODIFIED BETWEEN '" & InpDate & "' AND '" & CurrEDate & "' GROUP BY QABID ) AS TS4 "
        strQuery = strQuery & " ON TS4.QABID = U.USERID"
        ...
        strQuery = strQuery & " WHERE T.STATUS = '1073741824' and TS.TRANSCRIPTIONID = T.TRANSCRIPTIONID AND TS.USERLEVEL = '8192'  "
        ...
        strQuery = strQuery & " LEFT JOIN (SELECT MTBID, SUM(STDBODYLC) AS TSLINES FROM AdminSecureweb.dbo.TBLDICTATIONDETAILS "
        strQuery = strQuery & " WHERE DATEMODIFIED BETWEEN '" & InpDate & "' AND '" & CurrEDate & "' GROUP BY MTBID ) AS TS6 "
        strQuery = strQuery & " ON TS6.MTBID = U.USERID"
        strQuery = strQuery & " LEFT JOIN (SELECT QABSEID, SUM(STDBODYLC) AS TSLINES FROM AdminSecureweb.dbo.TBLDICTATIONDETAILS "
        strQuery = strQuery & " WHERE DATEMODIFIED BETWEEN '" & InpDate & "' AND '" & CurrEDate & "' GROUP BY QABSEID ) AS TS7 "
        strQuery = strQuery & " ON TS7.QABSEID = U.USERID"
        strQuery = strQuery & " LEFT JOIN (SELECT MTID, SUM(STDBODYLC) AS TSLINES FROM AdminSecureweb.dbo.TBLDICTATIONDETAILS "
        strQuery = strQuery & " WHERE DATEMODIFIED BETWEEN '" & CurrSDate & "' AND '" & CurrEDate & "' GROUP BY MTID ) AS TS8 "
        strQuery = strQuery & " ON TS8.MTID = U.USERID"
        strQuery = strQuery & " LEFT JOIN (SELECT MTPLUSID, SUM(STDBODYLC) AS TSLINES FROM AdminSecureweb.dbo.TBLDICTATIONDETAILS "
        strQuery = strQuery & " WHERE DATEMODIFIED BETWEEN '" & CurrSDate & "' AND '" & CurrEDate & "' GROUP BY MTPLUSID ) AS TS9 "
        strQuery = strQuery & " ON TS9.MTPLUSID = U.USERID"
        strQuery = strQuery & " LEFT JOIN (SELECT QAID, SUM(STDBODYLC) AS TSLINES FROM AdminSecureweb.dbo.TBLDICTATIONDETAILS "
        strQuery = strQuery & " WHERE DATEMODIFIED BETWEEN '" & CurrSDate & "' AND '" & CurrEDate & "' GROUP BY QAID ) AS TS10 "
        strQuery = strQuery & " ON TS10.QAID = U.USERID"
        strQuery = strQuery & " LEFT JOIN (SELECT QABID, SUM(STDBODYLC) AS TSLINES FROM AdminSecureweb.dbo.TBLDICTATIONDETAILS "
        strQuery = strQuery & " WHERE DATEMODIFIED BETWEEN '" & CurrSDate & "' AND '" & CurrEDate & "' GROUP BY QABID ) AS TS11 "
        strQuery = strQuery & " ON TS11.QABID = U.USERID"
        strQuery = strQuery & " LEFT JOIN (SELECT TS.USERID, SUM(TS.LINECOUNT) AS TSLINES FROM TBLTRANSCRIPTIONSTATUS TS, TBLTRANSCRIPTIONMAIN T"
        strQuery = strQuery & " WHERE T.STATUS = '1073741824' and TS.TRANSCRIPTIONID = T.TRANSCRIPTIONID AND TS.USERLEVEL = '8192'  "
        ...
        strQuery = strQuery & " LEFT JOIN (SELECT MTBID, SUM(STDBODYLC) AS TSLINES FROM AdminSecureweb.dbo.TBLDICTATIONDETAILS "
        strQuery = strQuery & " WHERE DATEMODIFIED BETWEEN '" & CurrSDate & "' AND '" & CurrEDate & "' GROUP BY MTBID ) AS TS13 "
        strQuery = strQuery & " ON TS13.MTBID = U.USERID"
        strQuery = strQuery & " LEFT JOIN (SELECT QABSEID, SUM(STDBODYLC) AS TSLINES FROM AdminSecureweb.dbo.TBLDICTATIONDETAILS "
        strQuery = strQuery & " WHERE DATEMODIFIED BETWEEN '" & CurrSDate & "' AND '" & CurrEDate & "' GROUP BY QABSEID ) AS TS14 "
        strQuery = strQuery & " ON TS14.QABSEID = U.USERID"
        strQuery = strQuery & " Where  (U.Isdeleted = 'False' or U.Isdeleted is NULL)  AND U.ProdMode in ('HBA','EMPLOYEE') "
        If DLUsers.SelectedValue <> "" Then
            strQuery = strQuery & " And U.USERID = '" & DLUsers.SelectedValue & "' "
        End If
        'strQuery = strQuery & " ORDER BY D.NAME"
        strQuery = strQuery & " UNION SELECT U.FIRSTNAME + ' ' + U.LASTNAME AS UNAME, U.USERNAME, U.TRGLINES, U.PRODMODE AS DNAME, ISNULL(A.ACCOUNTNAME, 'Direct Accounts') AS ACCOUNTNAME "
        strQuery = strQuery & " ,ISNULL(TS1.TSLINES,0) AS MTLINES"
        strQuery = strQuery & " ,ISNULL(TS2.TSLINES,0) AS MTPLINES"
        strQuery = strQuery & " ,ISNULL(TS3.TSLINES,0) AS QALINES"
        strQuery = strQuery & " ,ISNULL(TS4.TSLINES,0) AS BBLINES"
        ...
        If DLGroup.SelectedValue = "Platform" Then
            strQuery = strQuery & " ) T GROUP BY UNAME, USERNAME, TRGLINES, DNAME, ACCOUNTNAME ORDER BY ACCOUNTNAME, UNAME "
        Else
            strQuery = strQuery & " ) T GROUP BY UNAME, USERNAME, TRGLINES, DNAME, ACCOUNTNAME ORDER BY DNAME, UNAME "
        End If
        ...
        Try
            ...
            Dim objDA As New System.Data.SqlClient.SqlDataAdapter(strQuery, oConn)

            Dim objDS As New System.Data.DataSet()
            objDA.Fill(objDS)
            oConn.Close()
            For Each DRRecT As DataRow In objDS.Tables(0).Rows
                ...
                Cell19.Text = DRRecT(11).ToString
                Cell20.Text = DRRecT(12).ToString
                Cell21.Text = DRRecT(13).ToString
                Cell22.Text = DRRecT(14).ToString
                Cell23.Text = DRRecT(15).ToString
                Cell24.Text = DRRecT(16).ToString
                Cell25.Text = DRRecT(17).ToString
                ...
                STMTDMTLines += DRRecT(11).ToString
                STMTDMTPLines += DRRecT(12).ToString
                STMTDQALines += DRRecT(13).ToString
                STMTDPPQALines += DRRecT(14).ToString
                STMTDMTBLines += DRRecT(15).ToString
                STMTDQABLines += DRRecT(16).ToString
                STMTDQABSELines += DRRecT(17).ToString
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        'Response.Write("The page is Under Construction")
        'Response.End()

        If Not IsPostBack Then
            Dim strconn As String
            strconn = System.Configuration.ConfigurationManager.AppSettings("ETSCon")
            Dim SQLCmd As New SqlCommand("Select * from tblusers  Where (Isdeleted = 'False' or Isdeleted is NULL) order by firstname", New SqlConnection(strconn))
            Try
