...
Partial Class ViewUnitsTrend
    ...
    Protected Sub ShowActDetails()
        ...
        Dim InpSDate As Date
        Dim InpEDate As Date
        If TxtSDate.Text = "" Then
            strSDate = Now.ToShortDateString
        Else
            strSDate = TxtSDate.Text
        End If
        If TxtEDate.Text = "" Then
            strEDate = Now.ToShortDateString
        Else
            strEDate = TxtEDate.Text
        End If
        InpSDate = Date.Parse(strSDate)
        InpEDate = Date.Parse(strEDate)
        ...
        Dim AccIDs As String = String.Empty

        DayDiff = DateDiff(DateInterval.Day, InpSDate, InpEDate)
        Label7.Text = "Selected Accounts: "
        For Each LI As ListItem In DropDownList1.Items
            If LI.Selected Then
                If AccIDs = String.Empty Then
                    AccIDs = "'" & LI.Value & "'"
                    Label7.Text = Label7.Text & LI.Text
                Else
                    Label7.Text = Label7.Text & "," & LI.Text
                    AccIDs = AccIDs & ",'" & LI.Value & "'"
                End If
            End If
        Next
        'Response.Write(AccIDs)
        'Exit Sub

        Dim DTSet2 As System.Data.DataSet = objrep.GetProductivityPeriodReport(New Guid(Session("ContractorId").ToString), DayDiff, InpEDate, AccIDs)
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        filename = "Productivity by Period " & Month(Now) & Day(Now) & Year(Now) & ".xls"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            '    CheckBoxList1.Items(i).Attributes.Add("onclick", "GetCheckedValues()")

            'Next
            DropDownList1.Attributes.Add("onchange", "javascript:alert('copy');")
            ...
            End If
            objAct = Nothing
            TxtSDate.Text = Now.AddDays(-7).ToShortDateString
