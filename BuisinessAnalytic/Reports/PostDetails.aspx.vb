...
Partial Class MIS_DailyMins
    ...
    Protected Sub ShowActDetails()
        ...
        TBillOtherAmt = 0
        TBillTotAmount = 0
        strQuery = "Select A.Accountid, IsNull(V.OthCharges,0) as ActOthCharges, BA.Mode, A.Description, A.BillActNumber, C.priority, C.Description as CateDescr, BA.MinBilling, BA.BillAccID, BA.Posted, I.Amount as InvAmount, I.InvRecFound    "
        ...
        strQuery = strQuery & " LEFT OUTER JOIN (Select AccID, Amount, 'Yes' as InvRecFound from AdminSecureweb.dbo.InvupData where BillMonth='" & DLMonth.SelectedValue & "' and Billyear='" & DLYear.SelectedValue & "' and BillCycle='Cycle" & DLCycle.SelectedValue & "' ) I "
                ...
                Else
                    If DRRec("InvRecFound").ToString = "Yes" Then
                        ...
                        If -FormatNumber(DRRec("Invamount"), 2) = FormatNumber(BillAmt + DRRec("ActOthCharges"), 2) Then
                            Cell10.Text = "<input type=checkbox name=chBillAccID value=" & BillAccID & " onclick='highlightRow(this);'>"
