...
Partial Class Billing_LCMethods_LCMethodology
    ...
    Protected Sub tblsubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles tblsubmit.Click
        Dim strConn As String
        strConn = System.Configuration.ConfigurationManager.AppSettings("ETSCon")
        Dim SQLCmd As New SqlCommand("Select  A.AccountName, I.Descr, I.quantity, I.Amount, I.TotAmount, I.servicedate from AdminSecureweb.dbo.tblinvitemdet I, AdminETS.dbo.tblAccounts A where I.AccountID = A.accountID and month(ServiceDate) = '" & DLMonth.SelectedValue & "' and Year(ServiceDate) ='" & DLYear.SelectedValue & "' order by Accountname", New SqlConnection(strConn))
        Try
            SQLCmd.Connection.Open()
            '        Dim DRRec As SqlDataReader = SQLCmd.ExecuteReader()

            MyDataGrid.DataSource = SQLCmd.ExecuteReader()
            MyDataGrid.DataBind()
            '        SQLCmd.Connection.Close()
