...
Partial Class FormAddCust
    ...
    Protected Sub accname_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles ddaccname.SelectedIndexChanged
        Dim billmode As String = String.Empty
        ...
        While DR.Read()
            ...
            billmode = DR.GetValue(52).ToString
            Select Case Trim(billmode)
                Case "S"
                    txtloc.Text = "Standard"
                Case "DC"
                    txtloc.Text = "Dictatorwise"
                Case "DV"
                    txtloc.Text = "Devicewise"
                Case "LC"
                    txtloc.Text = "Locationwise"
                Case "TW"
                    txtloc.Text = "Templatewise"
            End Select
        ...
        Dim keypad As String = String.Empty
        con.Open()
        Dim keycmd As New SqlCommand("select top 1 * from tbltollfree where custid='" & custid & "'", con)
        Dim keyDR As SqlDataReader = keycmd.ExecuteReader()
        While keyDR.Read()
            keypad = keyDR.GetValue(4).ToString
            If keypad = "" Then
                txtpk.Text = "NA"
            Else
                txtpk.Text = keypad.ToString
            End If
