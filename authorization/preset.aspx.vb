...
Partial Class Passwordmodule_preset
    ...
    Protected Sub btn1_Click(sender As Object, e As System.EventArgs) Handles btn1.Click
        ...
        Else
            ...
            If hans.Value.ToLower() = uanswer.ToLower() Then
                GoTo here
            Else
                counter.Value = Val(counter.Value) + 1
                If counter.Value >= 3 Then
                    Label3.Text = "Does not match our records. You have had 3 unsuccessful attempts. Please contact Technical Support Desk to reset your registration details."
                ...
                Else
                    ...
                    Label3.Text = "Does not match our records. Please answer correctly </BR>or contact helpdesk at techsupport@edictate.com </BR>or Skype ID serversupportteam"
                    RandonAns()
                    Exit Sub
                End If
...
here:
            ...
            For Each row As DataRow In dtpcheck.Rows
                ...
                pass = base64Decode(row("password").ToString.Trim)
    ...
    Private Sub passconfirmmail(UFname As String, Email As String)
        ...
        myString = myString.Replace("$$BODY$$", "You've successfully changed your Secure-Scribe password. Please click <a href=""https://admin.securexsoft.com"">here</a> to login.")
    ...
    Public Function base64Decode(ByVal sData As String) As String
        ...
        Return result
    End Function
    Protected Sub RandonAns()
        Dim blSecurity As New ETS.BL.BALSecurity
        Dim uid As String = Request.QueryString("uid")
        Dim random As New Random()
        hvalue.Value = Val(hvalue.Value) + 1
        value = random.Next(1, 4)
        Dim DT As DataTable = blSecurity.getUserSecretQuestions(uid)
        'Response.Write("value" & value & "\")
        If DT.Rows.Count > 0 Then
            Dim DR As DataRow = DT.Rows(0)
            Label1.Text = DR.Item("q" & hvalue.Value).ToString
            hans.Value = DR.Item("s" & hvalue.Value).ToString
        End If
        'Response.Write(hvalue.Value)
    End Sub
    Protected Sub form1_Load(sender As Object, e As System.EventArgs) Handles form1.Load
