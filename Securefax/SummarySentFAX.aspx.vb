...
Partial Class FaxPlus_SummarySentFAX
    ...
    Public Sub LoadData()
        ...
        Try
            ...
            'Response.Write(varAccID.ToString & " " & varsDate.ToString & " " & vareDate.ToString)
            'Response.End()
            DS = clsFAX.GetSummarySecureFax(1, varAccID.ToString, varsDate.ToString, vareDate.ToString)

            'For Each dr As Data.DataRow In DS.Tables(0).Rows
            '    For Each DC As Data.DataColumn In DS.Tables(0).Columns
            '        Response.Write(DC.ColumnName & " - " & dr(DC.ColumnName).ToString() & "<BR>")
            '    Next
            'Next
            If DS.Tables.Count > 0 Then
                If DS.Tables(0).Rows.Count > 0 Then
