<%@ Page Language="VB" AutoEventWireup="false" CodeFile="VRSTemplateSearch.aspx.vb" Inherits="ets.Templates_TemplateSearh" %>
...
<script language="javascript" type="text/javascript">
    function btnEdit_onclick(tempID,TempName,UserID,Format)
    {
        document.all.btnEdit.disabled=true;
        var fpath;     //=ChkPath();
        var oShell = new ActiveXObject("WScript.Shell");  
        fpath=fpath+'\EditTemplate.exe';
        var ProFolder = oShell.ExpandEnvironmentStrings('%ProgramFiles%') + '\ETS\AMPTemplateEditor\EditTemplate.exe';
        var fpath1 = oShell.ExpandEnvironmentStrings('%ProgramFiles%') + '\\ETS\\AMPTemplateEditor\\EditTemplate.exe';
       //alert(fpath1);
        ...
        if( fso.FileExists(fpath) ) {
            if(fso.GetFileVersion(fpath)=='1.0.0.3') {
                ...
                oShell.Run('"' + fpath + '"' + ' ' + tempID + '|' + TempName + '|' + UserID + '|admin.securexsoft.com|'+Format, 1);        
            }else{
           ...
           if( fso.FileExists(fpath1) ) {
            if(fso.GetFileVersion(fpath1)=='1.0.0.3') {
                ...
                oShell.Run('"' + fpath1 + '"' + ' ' + tempID + '|' + TempName + '|' + UserID + '|admin.securexsoft.com|'+Format, 1);        
            }else{
                window.location='../TemplateEditor/Default.htm';
            }           
    ...
    <ContentTemplate>
        ...
        <asp:Panel ID="Panel2" runat="server" HorizontalAlign="Left">
            ...
            <tr bgcolor="#cccccc">         
                        ...
                        <INPUT TYPE="image" id="btnEdit" SRC="../App_Themes/Images/i_filter.gif" title="Edit Template" onclick="return btnEdit_onclick('<%#Container.DataItem("TemplateID")%>','<%#Container.DataItem("TemplateName")%>','<%#Session("UserID")%>','<%#container.Dataitem("Format")%>')" language="javascript" width="37" height="37">
