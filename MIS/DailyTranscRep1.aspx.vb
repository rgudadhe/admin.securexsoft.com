...
Partial Class MIS_DailyTranscRep1
    Inherits BasePage
    ...
    Protected Sub ShowActDetails(ByVal StartDate As Date, ByVal EndDate As Date, ByVal Export As Boolean)
        ...
        'strQuery = strQuery & " ,ISNULL(TS6.USERNAME, '') AS TS6ID,ISNULL(TS6.LINECOUNT, 0) AS TS6LINES, TS6.DATEMODIFIED AS TS6DATE"
        'strQuery = strQuery & " ,ISNULL(TS7.USERNAME, '') AS TS7ID,ISNULL(TS7.LINECOUNT, 0) AS TS7LINES, TS7.DATEMODIFIED AS TS7DATE, T.TYPE, TL.TemplateName"
        ...
        'strQuery = strQuery & " WHERE TS.USERID = U.USERID AND ((TS.USERLEVEL = '1' AND TS.STATUS IN ('2')) or (TS.USERLEVEL = '1' AND TS.STATUS IN ('8') and TS.TRANSCRIPTIONID IN (SELECT TRANSCRIPTIONID FROM TBLAUDITEPTL))) ) AS TS1 "
        'strQuery = strQuery & " ON TS1.TRANSCRIPTIONID = T.TRANSCRIPTIONID"
        'strQuery = strQuery & " LEFT JOIN (SELECT TS.TRANSCRIPTIONID, U.USERNAME, TS.LINECOUNT, TS.DATEMODIFIED FROM TBLTRANSCRIPTIONSTATUS TS, TBLUSERS U"
        'strQuery = strQuery & " WHERE TS.USERID = U.USERID AND TS.USERLEVEL = '1' AND TS.STATUS NOT IN ('2','8')) AS TS2 "
        'strQuery = strQuery & " ON TS2.TRANSCRIPTIONID = T.TRANSCRIPTIONID"
        'strQuery = strQuery & " LEFT JOIN (SELECT TS.TRANSCRIPTIONID, U.USERNAME, TS.LINECOUNT, TS.DATEMODIFIED FROM TBLTRANSCRIPTIONSTATUS TS, TBLUSERS U"
        'strQuery = strQuery & " WHERE TS.USERID = U.USERID AND ((TS.USERLEVEL = '2' and TS.STATUS NOT IN ('8')) OR ( TS.USERLEVEL = '8' AND TS.TRANSCRIPTIONID IN (SELECT TRANSCRIPTIONID FROM TBLAUDITEPTL)))  ) AS TS3 "
        'strQuery = strQuery & " ON TS3.TRANSCRIPTIONID = T.TRANSCRIPTIONID"
        'strQuery = strQuery & " LEFT JOIN (SELECT TS.TRANSCRIPTIONID, U.USERNAME, TS.LINECOUNT, TS.DATEMODIFIED FROM TBLTRANSCRIPTIONSTATUS TS, TBLUSERS U"
        'strQuery = strQuery & " WHERE TS.TRANSCRIPTIONID NOT IN (SELECT TRANSCRIPTIONID FROM TBLAUDITEPTL) AND TS.USERID = U.USERID AND TS.USERLEVEL = '2' and TS.STATUS = '8' ) AS TS4 "
        'strQuery = strQuery & " ON TS4.TRANSCRIPTIONID = T.TRANSCRIPTIONID"
        'strQuery = strQuery & " LEFT JOIN (SELECT      dbo.tblAuditRecords.TranscriptionID, dbo.tblUsers.UserName, dbo.tblAuditRecords.DateFinished as DATEMODIFIED, dbo.tblTransLC.stdBodyLC as LINECOUNT "
        'strQuery = strQuery & " FROM         dbo.tblAuditRecords INNER JOIN dbo.tblUsers ON dbo.tblAuditRecords.AuditorID = dbo.tblUsers.UserID LEFT OUTER JOIN "
        'strQuery = strQuery & " dbo.tblTransLC ON dbo.tblAuditRecords.TranscriptionID = dbo.tblTransLC.transcriptionid) AS TS5 "
        'strQuery = strQuery & " ON TS5.TRANSCRIPTIONID = T.TRANSCRIPTIONID"
        'strQuery = strQuery & " LEFT JOIN (SELECT TS.TRANSCRIPTIONID, U.USERNAME, TS.LINECOUNT, TS.DATEMODIFIED FROM TBLTRANSCRIPTIONSTATUS TS, TBLUSERS U"
        'strQuery = strQuery & " WHERE TS.USERID = U.USERID AND TS.TRANSCRIPTIONID NOT IN (SELECT TRANSCRIPTIONID FROM TBLAUDITEPTL)  AND"
        'strQuery = strQuery & " TS.USERLEVEL = '1' AND  TS.STATUS IN ('8') ) AS TS6 "
        'strQuery = strQuery & " ON TS6.TRANSCRIPTIONID = T.TRANSCRIPTIONID"
        'strQuery = strQuery & " LEFT JOIN (SELECT TS.TRANSCRIPTIONID, U.USERNAME, TS.LINECOUNT, TS.DATEMODIFIED FROM TBLTRANSCRIPTIONSTATUS TS, TBLUSERS U"
        'strQuery = strQuery & " WHERE TS.USERID = U.USERID AND TS.TRANSCRIPTIONID NOT IN (SELECT TRANSCRIPTIONID FROM TBLAUDITEPTL)  AND"
        'strQuery = strQuery & " TS.USERLEVEL = '8' ) AS TS7 "
        'strQuery = strQuery & " ON TS7.TRANSCRIPTIONID = T.TRANSCRIPTIONID"
        ...
        ''MyDataGrid.DataSource = DRRecT
        ''MyDataGrid.DataBind()

        Dim SQLCmdT As New SqlCommand("[ets].[dbo].[SP_TransReports]", New SqlConnection(strConn))
        SQLCmdT.CommandType = CommandType.StoredProcedure
        SQLCmdT.CommandTimeout = 3600
        ...
        Try
            ...
            If DRRecT.HasRows Then
                While DRRecT.Read
                    ...
                    Dim Cell32 As New TableCell
                    Dim Cell33 As New TableCell
                    Dim Cell34 As New TableCell
                    ...
                    Cell31.Text = DRRecT(31).ToString
                    Cell32.Text = DRRecT(32).ToString
                    Cell33.Text = DRRecT(33).ToString
                    Cell34.Text = DRRecT(34).ToString
                    ...
                    MTLines = MTLines + CInt(DRRecT(10).ToString)
                    MTPLines = MTPLines + CInt(DRRecT(13).ToString)
                    QALines = QALines + CInt(DRRecT(16).ToString)
                    BBLines = BBLines + CInt(DRRecT(19).ToString)
                    PPQALines = PPQALines + CInt(DRRecT(22).ToString)
                    MTBLines = QABLines + CInt(DRRecT(25).ToString)
                    QABLines = QABLines + CInt(DRRecT(28).ToString)
                End While
            End If
    ...
    Protected Sub ShowDictDetails(ByVal ActID As String, ByVal StartDate As Date, ByVal EndDate As Date, ByVal Export As Boolean)
        ...
        Dim SQLCmdT As New SqlCommand("[ets].[dbo].[SP_TransReportsByActID]", New SqlConnection(strConn))
        SQLCmdT.CommandType = CommandType.StoredProcedure
        ...
        SQLCmdT.Parameters("@EndDate").Value = EndDate
        SQLCmdT.Parameters("@HourDiff").Value = Hourdiff
        SQLCmdT.Parameters.Add("@ActID", SqlDbType.UniqueIdentifier)
        SQLCmdT.Parameters("@ActID").Value = New System.Guid(ActID)
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            Dim strconn As String
            strconn = System.Configuration.ConfigurationManager.AppSettings("ETSConOne")
            Dim SQLCmd As New SqlCommand("Select * from tblaccounts where (isdeleted is null or isdeleted = 'False')  and contractorid = '" & Session("contractorid").ToString & "' order by accountname", New SqlConnection(strconn))
            Try
                ...
                If DRRec.HasRows = True Then
                    While (DRRec.Read)
                        ...
                        LI.Text = DRRec("accountname").ToString
