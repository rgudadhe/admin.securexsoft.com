...
Partial Class MIS_DailyMins
    ...
    Protected Sub ShowActDetails(ByVal accountid As String, ByVal StartDate As Date, ByVal EndDate As Date, ByVal Export As Boolean)
        ...
        strQuery = "SELECT T.JOBNUMBER, T.CUSTJOBID, A.ACCOUNTNAME, P.FIRSTNAME + ' ' + P.LASTNAME AS PNAME, T.DATECREATED,  isnull(dateadd(hh, 9, dateadd(minute, 30, T.DATEMODIFIED)), T.DATEMODIFIED), T.DURATION, T.PRIORITY"
        strQuery = strQuery & " ,ISNULL(TS1.USERNAME, '') AS TS1ID,ISNULL(TS1.LINECOUNT, 0) AS TS1LINES, isnull(dateadd(hh, 9, dateadd(minute, 30, TS1.DATEMODIFIED)), TS1.DATEMODIFIED) AS TS1DATE"
        strQuery = strQuery & " ,ISNULL(TS2.USERNAME, '') AS TS2ID,ISNULL(TS2.LINECOUNT, 0) AS TS2LINES, isnull(dateadd(hh, 9, dateadd(minute, 30, TS2.DATEMODIFIED)), TS2.DATEMODIFIED) AS TS2DATE"
        strQuery = strQuery & " ,ISNULL(TS3.USERNAME, '') AS TS3ID,ISNULL(TS3.LINECOUNT, 0) AS TS3LINES, isnull(dateadd(hh, 9, dateadd(minute, 30, TS3.DATEMODIFIED)), TS3.DATEMODIFIED) AS TS3DATE, T.TYPE, TL.TemplateName, isnull(PL.LevelName,PL1.LevelName) as LevelName"
        ...
        strQuery = strQuery & " LEFT JOIN (SELECT TS.TRANSCRIPTIONID, U.USERID, U.USERNAME, TS.LINECOUNT, TS.DATEMODIFIED FROM TBLTRANSCRIPTIONSTATUS TS, TBLUSERS U"
        strQuery = strQuery & " WHERE TS.USERID = U.USERID AND TS.USERLEVEL = '1' ) AS TS1 "
        strQuery = strQuery & " ON TS1.TRANSCRIPTIONID = T.TRANSCRIPTIONID"
        strQuery = strQuery & " LEFT JOIN (SELECT TS.TRANSCRIPTIONID, U.USERID, U.USERNAME, TS.LINECOUNT, TS.DATEMODIFIED FROM TBLTRANSCRIPTIONSTATUS TS, TBLUSERS U"
        strQuery = strQuery & " WHERE TS.USERID = U.USERID AND TS.USERLEVEL = '2' ) AS TS2 "
        strQuery = strQuery & " ON TS2.TRANSCRIPTIONID = T.TRANSCRIPTIONID"
        strQuery = strQuery & " LEFT JOIN (SELECT TS.TRANSCRIPTIONID, U.USERID, U.USERNAME, TS.LINECOUNT, TS.DATEMODIFIED FROM TBLTRANSCRIPTIONSTATUS TS, TBLUSERS U"
        strQuery = strQuery & " WHERE TS.USERID = U.USERID AND TS.USERLEVEL = '8' ) AS TS3 "
        strQuery = strQuery & " ON TS3.TRANSCRIPTIONID = T.TRANSCRIPTIONID"
        strQuery = strQuery & " LEFT JOIN (Select case levelno when '1073741824' then levelname else 'Pending ' + levelname end as levelname, Levelno from tblProductionLevels) PL1 ON PL1.LevelNo = T.status "
        strQuery = strQuery & " LEFT JOIN (Select 'CheckedOut ' + levelname as levelname, Levelno from tblProductionLevels) PL ON PL.LevelNo = (T.status-100) "

        'strQuery = strQuery & " WHERE T.AccountID is not NULL "
        If accountid = "" Then
            strQuery = strQuery & " WHERE T.AccountID is not NULL "
        Else
            strQuery = strQuery & " WHERE T.AccountID ='" & accountid & "' "
        End If

        If DLStatus.SelectedValue = 1 Then
            If DLUsers.SelectedValue <> "" Then
                strQuery = strQuery & "  AND TS1.USERID = '" & DLUsers.SelectedValue & "'  and TS1.DATEMODIFIED BETWEEN '" & StartDate & "' and  '" & EndDate & "' "
            Else
                strQuery = strQuery & "    and TS1.DATEMODIFIED BETWEEN '" & StartDate & "' and  '" & EndDate & "' "
            End If
        ElseIf DLStatus.SelectedValue = 2 Then
            If DLUsers.SelectedValue <> "" Then
                strQuery = strQuery & "  AND TS2.USERID = '" & DLUsers.SelectedValue & "'  and TS2.DATEMODIFIED BETWEEN '" & StartDate & "' and  '" & EndDate & "' "
            Else
                strQuery = strQuery & "    and TS2.DATEMODIFIED BETWEEN '" & StartDate & "' and  '" & EndDate & "' "
            End If
        ElseIf DLStatus.SelectedValue = 8 Then
            If DLUsers.SelectedValue <> "" Then
                strQuery = strQuery & "  AND TS3.USERID = '" & DLUsers.SelectedValue & "'  and TS3.DATEMODIFIED BETWEEN '" & StartDate & "' and  '" & EndDate & "' "
            Else
                strQuery = strQuery & "    and TS3.DATEMODIFIED BETWEEN '" & StartDate & "' and  '" & EndDate & "' "
            End If
        Else
            strQuery = strQuery & "  AND T.STATUS = '1073741824' and  T.DATEMODIFIED BETWEEN '" & StartDate & "' and  '" & EndDate & "'"
        End If
        ...
        Try
            ...
            If DRRecT.HasRows Then
                While DRRecT.Read
                    ...
                    Dim Cell27 As New TableCell
                    Dim Cell28 As New TableCell
                    Dim Cell29 As New TableCell
                    Dim Cell30 As New TableCell
                    ...
                    'Cell23.Text = DRRecT(23).ToString
                    'Cell24.Text = DRRecT(24).ToString
                    'Cell25.Text = DRRecT(25).ToString
                    'Cell26.Text = DRRecT(26).ToString
                    'Cell27.Text = DRRecT(27).ToString
                    'Cell28.Text = DRRecT(28).ToString
                    'Cell29.Text = DRRecT(29).ToString
                    'Cell30.Text = DRRecT(30).ToString
                    ...
                    'Row1.Cells.Add(Cell26)
                    'Row1.Cells.Add(Cell27)
                    ...
                    QALines = QALines + CInt(DRRecT(12).ToString)
                    BBLines = BBLines + CInt(DRRecT(15).ToString)
            ...
            Dim CellT23 As New TableCell
            Dim CellT24 As New TableCell
            Dim CellT25 As New TableCell
            Dim CellT26 As New TableCell
            Dim CellT27 As New TableCell
            Dim CellT28 As New TableCell
            Dim CellT29 As New TableCell
            Dim CellT30 As New TableCell

            CellT0.ColumnSpan = 10
            ...
            CellT10.ColumnSpan = 2
            CellT10.Text = "Total QA Lines"
            CellT10.HorizontalAlign = HorizontalAlign.Right
            CellT12.Text = QALines
            CellT13.ColumnSpan = 2
            CellT13.Text = "Total BB Lines"
            CellT13.HorizontalAlign = HorizontalAlign.Right
            CellT15.Text = BBLines
            'CellT20.ColumnSpan = 2
            'CellT20.Text = "Total PPQA Lines"
            'CellT20.HorizontalAlign = HorizontalAlign.Right
            'CellT22.Text = PPQALines
            'CellT19.Text = ""
            'CellT23.ColumnSpan = 2
            'CellT23.Text = "Total MTB Lines"
            'CellT23.HorizontalAlign = HorizontalAlign.Right
            'CellT25.Text = MTBLines
            'CellT26.ColumnSpan = 2
            'CellT26.Text = "Total QAB Lines"
            'CellT26.HorizontalAlign = HorizontalAlign.Right
            'CellT28.Text = QABLines
            ...
            'RowT1.Cells.Add(CellT23)
            'RowT1.Cells.Add(CellT25)
            'RowT1.Cells.Add(CellT26)
            'RowT1.Cells.Add(CellT28)
    ...
    Protected Sub ShowDictDetails(ByVal ActID As String, ByVal StartDate As Date, ByVal EndDate As Date, ByVal Export As Boolean)
        ...
        strQuery = strQuery & " LEFT JOIN TBLACCOUNTS A ON T.ACCOUNTID = A.ACCOUNTID"
        strQuery = strQuery & " LEFT JOIN TBLPHYSICIANS P ON T.DICTATORID = P.PHYSICIANID"
        strQuery = strQuery & " LEFT JOIN (SELECT TS.TRANSCRIPTIONID, U.USERNAME, TS.LINECOUNT, TS.DATEMODIFIED FROM TBLTRANSCRIPTIONLOG TS, TBLUSERS U"
        strQuery = strQuery & " WHERE  TS.datemodified=(select max(datemodified)from tbltranscriptionlog where TranscriptionID= TS.TranscriptionID and TS.userlevel = userlevel) and TS.USERID = U.USERID AND TS.USERLEVEL = '1' AND TS.STATUS IN ('2') ) AS TS1 "
        strQuery = strQuery & " ON TS1.TRANSCRIPTIONID = T.TRANSCRIPTIONID"
        strQuery = strQuery & " LEFT JOIN (SELECT TS.TRANSCRIPTIONID, U.USERNAME, TS.LINECOUNT, TS.DATEMODIFIED FROM TBLTRANSCRIPTIONLOG TS, TBLUSERS U"
        strQuery = strQuery & " WHERE  TS.datemodified=(select max(datemodified)from tbltranscriptionlog where TranscriptionID= TS.TranscriptionID and TS.userlevel = userlevel) and TS.USERID = U.USERID AND TS.USERLEVEL = '1' AND TS.STATUS NOT IN ('2')) AS TS2 "
        strQuery = strQuery & " ON TS2.TRANSCRIPTIONID = T.TRANSCRIPTIONID"
        strQuery = strQuery & " LEFT JOIN (SELECT TS.TRANSCRIPTIONID, U.USERNAME, TS.LINECOUNT, TS.DATEMODIFIED FROM TBLTRANSCRIPTIONLOG TS, TBLUSERS U"
        strQuery = strQuery & " WHERE  TS.datemodified=(select max(datemodified)from tbltranscriptionlog where TranscriptionID= TS.TranscriptionID and TS.userlevel = userlevel) and TS.USERID = U.USERID AND TS.USERLEVEL = '2') AS TS3 "
        strQuery = strQuery & " ON TS3.TRANSCRIPTIONID = T.TRANSCRIPTIONID"
        strQuery = strQuery & " LEFT JOIN (SELECT TS.TRANSCRIPTIONID, U.USERNAME, TS.LINECOUNT, TS.DATEMODIFIED FROM TBLTRANSCRIPTIONLOG TS, TBLUSERS U"
        strQuery = strQuery & " WHERE  TS.datemodified=(select max(datemodified)from tbltranscriptionlog where TranscriptionID= TS.TranscriptionID and TS.userlevel = userlevel) and TS.USERID = U.USERID AND TS.USERLEVEL = '8') AS TS4 "
        strQuery = strQuery & " ON TS4.TRANSCRIPTIONID = T.TRANSCRIPTIONID"
        strQuery = strQuery & " LEFT JOIN (SELECT TS.TRANSCRIPTIONID, U.USERNAME, TS.LINECOUNT, TS.DATEMODIFIED FROM TBLTRANSCRIPTIONLOG TS, TBLUSERS U"
        strQuery = strQuery & " WHERE  TS.datemodified=(select max(datemodified)from tbltranscriptionlog where TranscriptionID= TS.TranscriptionID and TS.userlevel = userlevel) and TS.USERID = U.USERID AND TS.USERLEVEL = '8192') AS TS5 "
        strQuery = strQuery & " ON TS5.TRANSCRIPTIONID = T.TRANSCRIPTIONID"
        strQuery = strQuery & " WHERE "
        strQuery = strQuery & "  T.STATUS = '1073741824' and  T.DATEMODIFIED  BETWEEN '" & StartDate & "' and  '" & EndDate & "'"
        'strQuery = strQuery & " AND TS1.USERNAME IS NOT NULL"
        '       strQuery = strQuery & " AND T.STATUS = '1073741824' "
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            CalendarExtender2.SelectedDate = Now.AddDays(-1).ToShortDateString
            'TXTEDate.Text = Now.ToShortDateString
            'CalendarExtender1.SelectedDate = Now.ToShortDateString
            Dim sdate As Date
            Dim edate As Date
            Dim strconn As String
            strconn = System.Configuration.ConfigurationManager.AppSettings("ETSCon")
        ...
        Else
            If Request("txtsdate") <> "" Then
                CalendarExtender2.SelectedDate = Request("txtsdate")
            End If
            If Request("txtedate") <> "" Then
                CalendarExtender1.SelectedDate = Request("txtedate")
            End If
    ...
    Protected Sub tblsubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles tblsubmit.Click
        ...
        End If
        sDate = sDate.AddHours(-9)
        sDate = sDate.AddMinutes(-30)
        eDate = eDate.AddHours(-9)
    ...
    Private Sub ExpResult()
        ...
        'Else
        ShowActDetails(AccountID, sDate, eDate, True)
        'End If
