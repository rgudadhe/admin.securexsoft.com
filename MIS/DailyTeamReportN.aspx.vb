...
Partial Class MIS_DailyMins
    ...
    Protected Sub ShowActDetails()
        ...
        strQuery = strQuery & " Select U.FIRSTNAME + ' ' + U.LASTNAME AS UNAME, U.USERNAME, U.TRGLINES, D.NAME AS DNAME, ISNULL(A.ACCOUNTNAME, 'Direct Accounts') AS "
        strQuery = strQuery & " ACCOUNTNAME, ISNULL(MT.Lines,0) + ISNULL(MT1.Lines,0) as MTLines, ISNULL(MTB.Lines,0) as MTBLines, ISNULL(MTP.Lines,0) as MTPLines, "
        strQuery = strQuery & " ISNULL(QA.Lines,0) + ISNULL(QA1.Lines,0) as QALines, ISNULL(QABSE.Lines,0) as QABSELines, ISNULL(QAB.Lines,0) as BBLines, ISNULL(PPQA.Lines,0) as PPQALines "
        strQuery = strQuery & " , ISNULL(TMT.Lines,0)+ISNULL(TMT1.Lines,0) as TMTLines, ISNULL(TMTB.Lines,0) as TMTBLines, ISNULL(TMTP.Lines,0) as TMTPLines, ISNULL(TQA.Lines,0) + "
        strQuery = strQuery & " ISNULL(TQA1.Lines,0) as TQALines, ISNULL(TQABSE.Lines,0) as TQABSELines, ISNULL(TQAB.Lines,0) as TBBLines, ISNULL(TPPQA.Lines,0) as TPPQALines from tblusers U"
        ...
        strQuery = strQuery & " Left Outer Join (Select UserID, sum(Lines) as Lines From TmpTranLines where (Userlevel =1 and Status ='2') "
        strQuery = strQuery & " and dateM = '" & InpDate & "' group by UserID ) MT ON MT.USerID = U.USerID"
        strQuery = strQuery & " Left Outer Join"
        strQuery = strQuery & " (Select UserID, sum(Lines) as Lines From TmpTranLines where ( Userlevel =1 and Status ='8' and RecFound='Yes')"
        strQuery = strQuery & " and dateM = '" & InpDate & "' group by UserID ) MT1 ON MT1.USerID = U.USerID"
        strQuery = strQuery & " Left Outer Join"
        strQuery = strQuery & " (Select UserID, sum(Lines) as Lines From TmpTranLines where  Userlevel =1 and Status ='8' and RecFound is NULL"
        strQuery = strQuery & " and dateM = '" & InpDate & "' group by UserID ) MTB ON MTB.USerID = U.USerID"
        strQuery = strQuery & " Left Outer Join"
        strQuery = strQuery & " (Select UserID, sum(Lines) as Lines From TmpTranLines where Userlevel =1 and Status not in ('2','8')"
        strQuery = strQuery & " and dateM = '" & InpDate & "' group by UserID ) MTP ON MTP.USerID = U.USerID"
        strQuery = strQuery & " Left Outer Join"
        strQuery = strQuery & " (Select UserID, sum(Lines) as Lines From TmpTranLines where  (Userlevel =2 and Status not in ('8'))"
        strQuery = strQuery & " and dateM = '" & InpDate & "' group by UserID ) QA ON QA.USerID = U.USerID"
        strQuery = strQuery & " Left Outer Join"
        strQuery = strQuery & " (Select UserID, sum(Lines) as Lines From TmpTranLines where  (Userlevel =8 and RecFound ='Yes')"
        strQuery = strQuery & " and dateM = '" & InpDate & "' group by UserID ) QA1 ON QA1.USerID = U.USerID"
        strQuery = strQuery & " Left Outer Join"
        strQuery = strQuery & " (Select UserID, sum(Lines) as Lines From TmpTranLines where  Userlevel =2 and Status ='8' and RecFound is NULL"
        strQuery = strQuery & " and dateM = '" & InpDate & "' group by UserID ) QABSE ON QABSE.USerID = U.USerID"
        strQuery = strQuery & " Left Outer Join"
        strQuery = strQuery & " (Select UserID, sum(Lines) as Lines From TmpTranLines where Userlevel =8 and RecFound is NULL"
        strQuery = strQuery & " and dateM = '" & InpDate & "' group by UserID ) QAB ON QAB.USerID = U.USerID"
        strQuery = strQuery & " LEFT OUTER JOIN"
        strQuery = strQuery & " (SELECT      A.AuditorID, sum(lC.stdBodyLC) as Lines   FROM   tblAuditRecords A INNER JOIN tblTransLC LC ON A.TranscriptionID = LC.transcriptionid"
        strQuery = strQuery & " 	where A.DateFinished between '" & InpDate & "' and '" & CurrEDate & "' Group by  A.AuditorID) PPQA ON PPQA.AuditorID = U.UserID"
        strQuery = strQuery & " Left Outer Join"
        strQuery = strQuery & " (Select UserID, sum(Lines) as Lines From TmpTranLines where (Userlevel =1 and Status ='2')"
        strQuery = strQuery & " and dateM between '" & CurrSDate & "' and '" & CurrEDate & "' group by UserID ) TMT ON TMT.USerID = U.USerID"
        strQuery = strQuery & " Left Outer Join"
        strQuery = strQuery & " (Select UserID, sum(Lines) as Lines From TmpTranLines where ( Userlevel =1 and Status ='8' and RecFound='Yes')"
        strQuery = strQuery & " and dateM between '" & CurrSDate & "' and '" & CurrEDate & "' group by UserID ) TMT1 ON TMT1.USerID = U.USerID"
        strQuery = strQuery & " Left Outer Join"
        strQuery = strQuery & " (Select UserID, sum(Lines) as Lines From TmpTranLines where  Userlevel =1 and Status ='8' and RecFound is NULL"
        strQuery = strQuery & " and dateM between '" & CurrSDate & "' and '" & CurrEDate & "' group by UserID ) TMTB ON TMTB.USerID = U.USerID"
        strQuery = strQuery & " Left Outer Join"
        strQuery = strQuery & " (Select UserID, sum(Lines) as Lines From TmpTranLines where Userlevel =1 and Status not in ('2','8')"
        strQuery = strQuery & " and dateM between '" & CurrSDate & "' and '" & CurrEDate & "' group by UserID ) TMTP ON TMTP.USerID = U.USerID"
        strQuery = strQuery & " Left Outer Join"
        strQuery = strQuery & " (Select UserID, sum(Lines) as Lines From TmpTranLines where (Userlevel =2 and Status not in ('8'))"
        strQuery = strQuery & " and dateM between '" & CurrSDate & "' and '" & CurrEDate & "' group by UserID ) TQA ON TQA.USerID = U.USerID"
        strQuery = strQuery & " Left Outer Join"
        strQuery = strQuery & " (Select UserID, sum(Lines) as Lines From TmpTranLines where  (Userlevel =8 and RecFound ='Yes')"
        strQuery = strQuery & " and dateM between '" & CurrSDate & "' and '" & CurrEDate & "' group by UserID ) TQA1 ON TQA1.USerID = U.USerID"
        strQuery = strQuery & " Left Outer Join"
        strQuery = strQuery & " (Select UserID, sum(Lines) as Lines From TmpTranLines where  Userlevel =2 and Status ='8' and RecFound is NULL"
        strQuery = strQuery & " and dateM between '" & CurrSDate & "' and '" & CurrEDate & "' group by UserID ) TQABSE ON TQABSE.USerID = U.USerID"
        strQuery = strQuery & " Left Outer Join"
        strQuery = strQuery & " (Select UserID, sum(Lines) as Lines From TmpTranLines where Userlevel =8 and RecFound is NULL"
        strQuery = strQuery & " and dateM between '" & CurrSDate & "' and '" & CurrEDate & "' group by UserID ) TQAB ON TQAB.USerID = U.USerID"
        strQuery = strQuery & " LEFT OUTER JOIN"
        strQuery = strQuery & " (SELECT      A.AuditorID, sum(lC.stdBodyLC) as Lines   FROM   tblAuditRecords A INNER JOIN tblTransLC LC ON A.TranscriptionID = LC.transcriptionid"
        strQuery = strQuery & " 	where A.DateFinished between '" & CurrSDate & "' and '" & CurrEDate & "' Group by  A.AuditorID) TPPQA ON TPPQA.AuditorID = U.UserID"
        strQuery = strQuery & " WHERE  u.DEPARTMENTID = '2D0892BB-BF03-4652-83E3-B517EF7BA2CE' "
