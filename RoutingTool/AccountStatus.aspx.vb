...
Partial Class RoutingTool_Default
    ...
    Sub DictStatus()
        ...
        Try
            ...
            If DRRec.HasRows Then
                While (DRRec.Read)
                    ...
                    LevelNo = DRRec("LevelNo").ToString
        ...
        If HAccID.Value = "" Then
            strQuery = "Select distinct T.TranscriptionID,  T.Jobnumber, T.TAT, T.Priority, T.submitDate, T.DueDate, A.AccountName, T.duration, datediff(s,0,T.duration) as Mins, P.uname, TL.TemplateName, PL.LevelName, '' as username, '' as direct, T.datemodified from tbltranscriptionmain T LEFT JOIN tblProductionLevels PL ON PL.LevelNo = T.status LEFT JOIN (Select AccountID, AccountName from tblAccounts) AS A ON A.accountID=T.AccountID  LEFT JOIN (Select PhysicianID, firstname + ' ' + lastname as uname from tblphysicians) AS P ON P.physicianID=T.dictatorID   LEFT JOIN (Select TemplateID, TemplateName from tblTemplates) AS TL ON TL.TemplateID=T.TemplateID where PL.contractorid='" & Session("contractorid") & "' and T.contractorid='" & Session("contractorid") & "' and  T.status in (" & ProLevel.Value & ") order by  t.duedate "
        Else
            strQuery = "Select distinct T.TranscriptionID,  T.Jobnumber, T.TAT, T.Priority, T.submitDate, T.DueDate, A.AccountName, T.duration, datediff(s,0,T.duration) as Mins, P.uname, TL.TemplateName, PL.LevelName, '' as username, '' as direct, T.datemodified from tbltranscriptionmain T LEFT JOIN tblProductionLevels PL ON PL.LevelNo = T.status LEFT JOIN (Select AccountID, AccountName from tblAccounts) AS A ON A.accountID=T.AccountID  LEFT JOIN (Select PhysicianID, firstname + ' ' + lastname as uname from tblphysicians) AS P ON P.physicianID=T.dictatorID   LEFT JOIN (Select TemplateID, TemplateName from tblTemplates) AS TL ON TL.TemplateID=T.TemplateID where PL.contractorid='" & Session("contractorid") & "' and T.contractorid='" & Session("contractorid") & "' and T.AccountID = '" & HAccID.Value & "' and T.status in (" & ProLevel.Value & ")  order by  t.duedate "
        End If
    ...
    Protected Sub DLStatus_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles DLStatus.SelectedIndexChanged
        ...
        ElseIf DLStatus.SelectedValue = "Assign" Then
            ...
            strConn = System.Configuration.ConfigurationManager.AppSettings("ETSCon")
            Dim strQuery As String
            strQuery = "Select U.firstname + ' ' + U.lastname + '(' + username +')' as uname, U.userid from tblusers  U INNER JOIN tblUsersLevels UL ON  UL.UserID = U.USerID where   (U.Isdeleted is null or U.isdeleted = 'False') and dbo.chkLevel(UL.ProductionLevel, " & ProLevel.Value & ")='True' order by u.firstname "
            Dim CMUsr As New SqlCommand(strQuery, New SqlConnection(strConn))
            Try
                CMUsr.Connection.Open()
                Dim DRRec As SqlDataReader = CMUsr.ExecuteReader()
            ...
            If DLChoice.SelectedValue <> "" Then
                ...
                For I = 1 To TotJobs.Value
                    ...
                    If SelTransID <> "" Then
                        ...
                        DRExc.Transaction = Trans1
                        RecUp1 = DRExc.ExecuteNonQuery()
                        strQuery = "Insert Into tblTranscriptionLog (userid, TranscriptionID,UserLevel, Status, Datemodified, IP,AssignedBy) Values ('" & DLChoice.SelectedValue & "', '" & SelTransID & "', " & ProLevel.Value & ", " & realStatus & ",'" & Now & "', '" & Request.UserHostAddress() & "' ,'" & Session("UserID") & "') "
                        Dim DRExc1 As New SqlCommand(strQuery, SQLConn)
                        DRExc1.Transaction = Trans1
                        RecUp2 = DRExc1.ExecuteNonQuery()
