...
Partial Class RoutingTool_Default
    ...
    Sub DictStatus()
        ...
        Try
            ...
            DSRec = clsRo.RoutingToolJobRoutingRecordsByUsrIDByLevel(ProLevel.Value, Lvlchk, HUserID.Value, Session("ContractorID"))
            If DSRec.Tables.Count > 0 Then
                If DSRec.Tables(0).Rows.Count > 0 Then
                    For Each JobDR As Data.DataRow In DSRec.Tables(0).Rows
                        Mins = 0
    ...
    Sub AssignUserNew()
        ...
        Try
            clsPL = New ETS.BL.ProductionLevels
            clsPL.ContractorID = Session("ContractorID").ToString
            clsPL.LevelNo = CInt(ProLevel.Value) - 100
        ...
        Try
            clsRo = New ETS.BL.Routing
            DS = clsRo.RoutingToolEmpStatusByLevelByUsrID(CInt(ProLevel.Value) - 100, HAccID.Value, ProcStartDate, hourdiff, ServStartDate, ServEndDate, Session("ContractorID"), HUserID.Value)
            ...
            If DS.Tables.Count > 0 Then
                If DS.Tables(0).Rows.Count > 0 Then
                    For Each DRRec1 As Data.DataRow In DS.Tables(0).Rows
                        ...
                        Else
                            MinsAssn = FormatNumber((DRRec1("CHMins").ToString / 60), 0)
                            CL5.Text = "<a href='JobsRouting.aspx?Userid=" & DRRec1("Userid").ToString & "&ProLevel=" & ProLevel.Value & " '  Target=_Blank>" & FormatNumber((DRRec1("CHMins").ToString / 60), 0) & "</a>"
                        End If
                        ...
                        If IsDBNull(DRRec1("CHMins1")) Then
                            CellC.Text = 0
                        Else
                            CellC.Text = "<a href='JobsRouting.aspx?Userid=" & DRRec1("Userid").ToString & "&ProLevel=" & ProLevel.Value & " '  Target=_Blank>" & FormatNumber((DRRec1("CHMins1").ToString / 60), 0) & "</a>"
                        End If
                        RW1.Cells.Add(CellC)
