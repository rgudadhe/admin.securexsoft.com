Partial Class Transcend_HBA
    ...
    Protected Sub BindData()
        ...
        Try
            Dim objDataAdapter As New Data.SqlClient.SqlDataAdapter("SELECT AccID,AccName FROM Transcend.dbo.tblAccounts WHERE IsDeleted IS NULL AND Type='eScription' ORDER BY AccName ", System.Configuration.ConfigurationManager.AppSettings("ETSCon"))
            Dim objDS As New Data.DataSet
            objDataAdapter.Fill(objDS)
            ddlAccounts.DataSource = objDS.Tables(0)
            ddlAccounts.DataTextField = "AccName"
            ddlAccounts.DataValueField = "AccID"
            ddlAccounts.DataBind()
            ddlAccounts.Items.Insert(0, New ListItem("Please Select", String.Empty))
        Catch ex As Exception
        End Try
    End Sub
    Protected Sub btnSubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
        ...
        If ddlStatus.SelectedIndex > 0 And ddlQStatus.SelectedIndex > 0 Then
            lblMsg.Text = String.Empty
        ...
        Try
            If Not String.IsNullOrEmpty(hdnUserID.Value) Then
                If CheckTransID() Then
                    ...
                    varAccName = ddlAccounts.Items(ddlAccounts.SelectedIndex).Value.ToString

                    Dim varVRS As String = String.Empty
                    varVRS = ddlVRS.Items(ddlVRS.SelectedIndex).Value.ToString

                    Dim varJobNo As String = String.Empty
                    varJobNo = Trim(txtJobNo.Text)

                    Dim varLines As Double = 0
                    If Not String.IsNullOrEmpty(txtLines.Text) Then
                        varLines = CDbl(Trim(txtLines.Text))
                    End If
                    ...
                    If String.IsNullOrEmpty(varStatus) And ddlQStatus.SelectedIndex > 0 Then
                    ...
                    If Not String.IsNullOrEmpty(varStatus) Then
                        If CheckMTStatus() Then
                            lblMsg.Text = String.Empty
                            lblMsg.Text = "MT Status already updated for this job no"
                            Exit Sub
                        End If
                    End If
                    ...
                    If Not String.IsNullOrEmpty(varStatus) Or Not String.IsNullOrEmpty(varQStatus) Then
                        'Dim varFinalStatus As String = String.Empty
                        'varFinalStatus = ddlStatus.Items(ddlStatus.SelectedIndex).Value.ToString
                        If String.IsNullOrEmpty(varAccName.ToString) Or String.IsNullOrEmpty(varVRS.ToString) Then
                            lblMsg.Text = String.Empty
                            lblMsg.Text = "Please select account and vrs for e-Scription job"
                            EmptyFields()
                            Exit Sub
                        End If
                        ...
                        varQuery = "INSERT INTO TRANSCEND.DBO.tblLinesData (UserID" & IIf(String.IsNullOrEmpty(varAccName), String.Empty, ",AccID") & " ,JobNo " & IIf(varLines > 0, ",Lines", String.Empty) & "" & IIf(String.IsNullOrEmpty(varStatus), String.Empty, ",Status") & "" & IIf(String.IsNullOrEmpty(varQStatus.ToString), String.Empty, ",QAStatus") & "" & IIf(String.IsNullOrEmpty(varVRS), String.Empty, ",VRS") & " ,UpdatedOn ) VALUES ('" & hdnUserID.Value.ToString & "'" & IIf(String.IsNullOrEmpty(varAccName), String.Empty, "," & "'" & varAccName.ToString & "'") & " ,'" & varJobNo.ToString & "'" & IIf(varLines > 0, "," & varLines & "", String.Empty) & "" & IIf(String.IsNullOrEmpty(varStatus.ToString), String.Empty, ",'" & varStatus.ToString & "'") & "" & IIf(String.IsNullOrEmpty(varQStatus.ToString), String.Empty, ",'" & varQStatus.ToString & "'") & "" & IIf(String.IsNullOrEmpty(varVRS), String.Empty, "," & varVRS.ToString) & ",'" & Now() & "')"
                        oConn.Open()
