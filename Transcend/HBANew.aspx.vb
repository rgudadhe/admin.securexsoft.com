Partial Class Transcend_HBA
            ...
            If Not String.IsNullOrEmpty(hdnUserID.Value) Then
                If CheckTransID() Then
                    ...
                    If Not String.IsNullOrEmpty(varStatus) Then
                        ...
                        varQuery = "INSERT INTO TRANSCEND.DBO.tblLinesDataQA (UserID,JobNo,Status,UpdatedOn) VALUES ('" & hdnUserID.Value.ToString & "'," & varJobNo.ToString & ",'" & varStatus.ToString & "','" & Now() & "')"
                        oConn.Open()
    ...
    Protected Function CheckMTStatus() As Boolean
        ...
        Try
            oConn.Open()
            Dim objCmd As New Data.SqlClient.SqlCommand("SELECT Count(*) AS 'Count' FROM TRANSCEND.DBO.tblLinesDataQA WHERE JobNo='" & Trim(txtJobNo.Text.ToString) & "' AND Status IS NOT NULL", oConn)
    ...
    Protected Sub SearchData(ByVal Sort As String, ByVal Dir As String)
        If Not String.IsNullOrEmpty(txtStartDate.Text) And Not String.IsNullOrEmpty(txtEndDate.Text) Then
            If Not String.IsNullOrEmpty(hdnUserID.Value) Then
                ...
                Try
                    Dim varQuery As String = String.Empty
                    varQuery = " SELECT L.JobNo,A.AccID,L.Status,AccName,PR_Lines,L.UpdatedON FROM TRANSCEND.DBO.tblLinesDataQA L LEFT OUTER JOIN TRANSCEND.DBO.tblBData B ON L.JobNo=B.VWJob LEFT OUTER JOIN TRANSCEND.DBO.tblAccounts A ON B.CustomerID=A.CustomerID WHERE UserID='" & hdnUserID.Value.ToString & "' AND L.UpdatedOn BETWEEN '" & txtStartDate.Text & "' AND '" & DateAdd(DateInterval.Day, 1, CDate(txtEndDate.Text)).ToString & "'  " & _
                               " ORDER BY " & Sort & " " & Dir & " "
    ...
    Protected Sub LnlExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LnlExport.Click
        If GrdViewData.Rows.Count > 0 Then
            ...
            Try
                ...
                SQLString = " SELECT L.JobNo,A.AccID,L.Status,AccName,PR_Lines,L.UpdatedON FROM TRANSCEND.DBO.tblLinesDataQA L LEFT OUTER JOIN TRANSCEND.DBO.tblBData B ON L.JobNo=B.VWJob LEFT OUTER JOIN TRANSCEND.DBO.tblAccounts A ON B.CustomerID=A.CustomerID WHERE UserID='" & hdnUserID.Value.ToString & "' AND L.UpdatedOn BETWEEN '" & txtStartDate.Text & "' AND '" & DateAdd(DateInterval.Day, 1, CDate(txtEndDate.Text)).ToString & "'  "
