    ...
    Protected Sub FillTable(ByVal varParamMonth, ByVal varParamYear, ByVal Str, ByVal Flag)
        ...
        Try
            If UCase(Trim(Str)) = UCase(Trim("True")) Then
                If Now.Date.Month = varParamMonth And Now.Date.Year Then
                    varIntDays = Now.Date.Day
                Else
                    varDtTempDate = DateAdd(DateInterval.Month, 1, DateSerial(varParamYear, varParamMonth, 1))
                    varIntMonthDay = (DateAdd(DateInterval.Day, -1, varDtTempDate)).Day
                        ...
                        If DSAtt.Tables(0).Rows.Count > 0 Then
                            ...
                            If oRecAttendance.HasRows Then
                                While oRecAttendance.Read
                                    k = (oRecAttendance.GetDateTime(oRecAttendance.GetOrdinal("AttDate"))).Day
                                    varArrAttDate(k) = (oRecAttendance.GetDateTime(oRecAttendance.GetOrdinal("AttDate"))).Day
