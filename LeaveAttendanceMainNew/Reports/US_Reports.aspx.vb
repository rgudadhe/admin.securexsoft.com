Imports MainModule
Partial Class LeaveAttendanceMainNew_Reports_USReports
    Inherits BasePage
    ...
    Protected Sub FillTableStatusEmp(ByVal StartDate As Date, ByVal EndDate As Date, ByVal Dept As String)
        ...
        Try
            ...
            Try
                ...
                DsTemp = clsLeave.GetUSLeaveDetailsforMonthByDept(Dept.ToString, Month(StartDate), Month(StartDate), Year(StartDate), Year(StartDate), varDtTodayDate, Session("ContractorID").ToString, Session("WorkGroupID"))

                ' Dim DTR As Data.DataRow() = DsTemp.Tables(0).Select("country='USA'")
                ...
                If objRecGetMonthlyLeaveStatus.HasRows Then
                    While objRecGetMonthlyLeaveStatus.Read
                        Dim eCountry As String = String.Empty
                        Try
                            eCountry = objRecGetMonthlyLeaveStatus.GetString(objRecGetMonthlyLeaveStatus.GetOrdinal("country"))
                        Catch ex As Exception

                        End Try


                        If eCountry = "USA" Then
                            ...
                            While varDtEdDate >= varDtStDate
                                If Month(varDtStDate) = Month(StartDate) And Year(varDtStDate) = Year(StartDate) Then
                                    If varStrLeaveType <> "" Then
                                        varStrLeaveStatus = varStrUserName & " " & varStrLeaveType ' & "(Leave " & objRecGetMonthlyLeaveStatus.GetString(objRecGetMonthlyLeaveStatus.GetOrdinal("Status")) & varStrLeaveType & ")"
                                        varStrLeaveStatus = IIf(objRecGetMonthlyLeaveStatus.GetString(objRecGetMonthlyLeaveStatus.GetOrdinal("Status")).Trim = "Approved", "<font face=""Trebuchet MS"" size=""2"" color=green>" & varStrLeaveStatus & "</font>", "<font face=""Trebuchet MS"" size=""2"">" & varStrLeaveStatus & "</font>") & "</br>"
                                    Else
                                    ...
                                    If varArrLeaveStatus(Day(varDtStDate)) <> "" Then
                                        varArrLeaveStatus(Day(varDtStDate)) = varArrLeaveStatus(Day(varDtStDate)) & "<b>*</b> " & varStrLeaveStatus
                                    Else
                                        varArrLeaveStatus(Day(varDtStDate)) = "<b>*</b> " & varStrLeaveStatus
                                    End If
